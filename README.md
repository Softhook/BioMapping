...
[`visualiser/live.html`](visualiser/live.html) — receives GPS + GSR from the Flipper's **Live Stream** mode over Bluetooth LE in real time, for watching a walk unfold on a laptop or phone as it happens. Also reachable from `index.html` via the **Live** tab in the top-right view switcher (Single Track / Collective / Live), which embeds this same page.
