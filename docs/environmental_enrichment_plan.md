...
- **Correlation Matrix** — for each environmental feature, the association with three arousal channels: momentary (max phasic in a trailing 1 s window), baseline (mean tonic), and arousal-response rate (peak count per 15 s bin). Features are the 8 continuous `osm_*` fields plus `osm_in_park` (0/1, point-biserial) plus the **EM Fog Index** (0–100, Sub-GHz RSSI derived — added only when some sample carries a reading). `osm_road_class` is multi-level categorical and stays out. Method depends on how many walks are active:
  ...
  - **Between-walk heterogeneity (I², 2026-09-04).** `metaCorrelation` derives Higgins & Thompson's I² alongside its DerSimonian–Laird `tau2` — the share of across-walk variation that's real disagreement between walks rather than each walk's own sampling noise. Surfaced per channel as a "· mixed" chip tag at I² ≥ 50% (exact % in the chip's tooltip), and factored into the plain-English interpretation: a negligible pooled *r* with high I² is reported as "inconsistent across walks", not "no link" — those are different findings a flat null-result reading would conflate. `NaN` when k < 3 (not enough groups to assess heterogeneity at all).
...
3. **Reliability**: unlike the original plan (which didn't address network failure), the client handles Overpass rate limits (HTTP 429/509 with `Retry-After` honored), gateway timeouts (504, exponential backoff with jitter, up to 3 retries), and request aborts, with user-facing progress messages at each stage. **Mirror fallback (2026-09-04):** `OverpassClient.ENDPOINTS` tries `overpass-api.de` then `overpass.kumi.systems` — but only on a *connection-level* failure (the host itself unreachable: DNS/TCP/CORS, surfaced by `fetch()` as a thrown `TypeError`, or a persistent timeout even after that endpoint's own retries). A real HTTP error from a live server (rate limit exhausted, malformed query, area too large) is reported immediately rather than retried on the mirror, since it would fail identically there. **Shared-fetch fallback (2026-09-04):** collective mode's single combined-area query (below) can still time out even when the union bbox fits under `AREA_CAP_KM2` — a large *area* isn't the only way to make a query expensive; a merely-under-cap area that's very dense (many walks clustered in one busy part of a city) can be too. `GSRUI.enrichTrack` now catches that failure and falls through to fetching each track's own, far smaller bbox individually (the same path already used for a genuinely spread-out collection) instead of aborting every track's enrichment over one oversized request.
...
17. **Named Map Features, Urban Typology & Annotation Text Analysis (PROPOSED 2026-09-14).**
    The current pipeline abstracts OSM geometry into purely numerical spatial proxies (`osm_dist_green`, `osm_building_density_50m`, `osm_road_class`), discarding the specific identity of urban space. Meanwhile, user peak labels (`setPeakLabel()`) remain passive visual text.
    - **Named Features from Existing Overpass Payload:** The query in `overpass_client.js` already fetches `way["highway"]`, `node/way["amenity"]`, `node/way["shop"]`, and `way/relation["leisure"]`. By retaining `tags.name` during `reconstructGeometries()`, each peak and cluster can resolve its street name (e.g., *"Kingsland High Street"*) and adjacent prominent POIs/venues (*"Rio Cinema"*, *"Dalston Junction Station"*).
    - **Named Arousal Places:** Enables `GSRArousalPlaces.buildPlaces()` (`arousal_places.js`) to automatically assign human-readable urban identities to clusters (e.g., *"Place 1: Junction of Kingsland High St & Dalston Lane"*) instead of abstract `P1..Pn` IDs.
    - **Corridor Arousal Leaderboards:** Aggregates mean phasic amplitude and peak density per named street across walks, identifying the most stimulating vs. restorative urban corridors.
    - **Textual Sentiment & Affect Circumplex:** Implements a client-side, zero-telemetry NLP lexicon scorer (AFINN/VADER) on user annotations to extract emotional **Valence** (pleasant vs. unpleasant). Pairing text valence with physiological arousal projects events into Russell's Circumplex Model of Affect (separating stress/fear from excitement/joy), with keyword clustering across 6 urban domains (traffic, acoustic, social, nature, architectural, physical). Full architectural specification in [`spatial_semantics_and_annotation_analysis_plan.md`](spatial_semantics_and_annotation_analysis_plan.md).
