...
// DOM Elements cache
let fileInput, dropZone, fileInfoBox, loadedFileName, loadedFileMeta, clearFileBtn;
let sliders = {};
let statFields = {};
let tableBody;
...
function loadDemoData() {
  ...
  let csvRows = ["Time (s),Conductance (uS)"];
  ...
  for (let i = 0; i < len; i++) {
    ...
    if (t >= 22.0 && t <= 22.6) {
      motionArtifact = 0.8 * sin((t - 22.0) * Math.PI / 0.6); // smooth half-sine bump
    }
    if (t >= 65.1 && t <= 65.3) {
      motionArtifact = -1.2 * sin((t - 65.1) * Math.PI / 0.2); // sharp contact drop
    }

    const totalVal = tonicValue + phasicValue + noiseVal + motionArtifact;
    csvRows.push(`${t.toFixed(2)},${totalVal.toFixed(5)}`);
  }
