...
// EFF Short Wordlist (1296 words) — used to render the GPS module's
// UBX-SEC-UNIQID chip serial as a human-readable 5-word phrase instead of
// a 12-hex-digit string (see ubx_poll_chip_id() in gps_uart.c). 1296^5 ==
// 6^20 >= 2^48, so 5 words losslessly cover the full 48-bit chip ID.
//
// Source: https://www.eff.org/files/2016/09/08/eff_short_wordlist_1.txt
// (EFF's "Dice-Generated Passphrases" short wordlist, licensed CC BY 3.0 US:
// https://www.eff.org/dice — words unmodified, reordered here into plain
// array-index order 0..1295, discarding the original 4-digit dice-roll key,
// which this code has no use for.)
//
// All 1296 words are <= 5 characters, so a fixed-width
// [6] slot (word + NUL) is smaller and simpler than a pointer table.
#define EFF_SHORT_WORDLIST_COUNT 1296
static const char eff_short_wordlist[EFF_SHORT_WORDLIST_COUNT][6] = {
