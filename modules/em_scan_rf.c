...
void em_scan_rf_init(void) {
    ...
    // em_scan_rf_tune_and_warmup() always calls idle() + set_frequency_and_path()
    // at the top of every visit, so this residual state is unconditionally reset
...
// em_scan_rf_fast_sweep_snapshot() below — idle, flush the RX FIFO, retune
// (with path — see the doc comment on that call below), start RX, then wait
// out the AGC/PLL settling transient before either function starts sampling.
...
void em_scan_rf_dwell_band(int band_index, float* out_peak_dbm) {
    ...
    // takes 2-4ms, so 22 iterations ran for 44-88ms instead of 22ms.
    // Checking real elapsed ticks makes the loop exit as soon as actual time
    // reaches EM_SCAN_DWELL_MS regardless of scheduling jitter.
    float peak = -127.0f;
...
// not the retune. One known-safe idle+flush+retune+rx cycle per
