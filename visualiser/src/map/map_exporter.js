...
class GSRMapExporter {
  ...
  static async exportToSvg(mgr) {
    ...
    // than being squared off against it. Growing the canvas here — instead of drawing
   ...
   *
   * The geometry itself — stitching raw Marching Squares segments into
   * closed rings, closing off ones cut off at the grid/map edge, and
   * detecting interior "no data" holes to cut out — lives in
   * ContourRingGeometry.buildIsobandRings. This method's job is purely the
   ...
   * depend on a viewer's hsl() support. Hex, rgb()/rgba() and named
