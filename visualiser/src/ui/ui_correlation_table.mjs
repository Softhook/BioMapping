...
export const CorrelationTableUI = {
  ...
  drawRegressionScatter(
    ...
    const plotL = padL,
      ...
      plotBt = height - padB;
    const mapX = (x) =>
      clamp(padL + ((x - minX) / rangeX) * (plotR - plotL), plotL, plotR);
    const mapY = (y) =>
      clamp(plotBt - ((y - minY) / rangeY) * (plotBt - plotT), plotT, plotBt);
    ...
    for (let i = 0; i < n; i++) {
      ...
      const jit = jitterAmp
        ? (((i * 2654435761) % 1000) / 1000 - 0.5) * 2 * jitterAmp
        : 0;
    ...
    if (isBinaryX) {
      ...
      for (const g of [0, 1]) {
        ...
        ctx.moveTo(cx - boxHalf, mapY(md));
        ctx.lineTo(cx + boxHalf, mapY(md));
        ctx.moveTo(cx, mapY(q3));
        ctx.lineTo(cx, mapY(w2)); // whiskers
        ctx.moveTo(cx, mapY(q1));
        ctx.lineTo(cx, mapY(w1));
        ctx.moveTo(cx - boxHalf * 0.5, mapY(w2));
        ctx.lineTo(cx + boxHalf * 0.5, mapY(w2));
        ctx.moveTo(cx - boxHalf * 0.5, mapY(w1));
        ctx.lineTo(cx + boxHalf * 0.5, mapY(w1));
        ctx.stroke();
      }
    } else {
  ...
  syncScatterEnvOptions(hasEmFog, hasSpeed = false, hasNdvi = false) {
    ...
    if (!sel) return;
    const opts = GSR_CONST.OSM_METRICS.filter(
  ...
  correlationBand(r) {
    const a = Math.abs(r);
    if (!(a >= 0.1)) return { key: 'negligible', label: 'negligible' };
    if (a < 0.2) return { key: 'small', label: 'small' };
    if (a < 0.3) return { key: 'moderate', label: 'moderate' };
  ...
  sortCorrelationTable(col) {
    ...
    if (AppState.corrSortColumn === col) {
      AppState.corrSortDirection =
        AppState.corrSortDirection === 'asc' ? 'desc' : 'asc';
    } else {
      AppState.corrSortColumn = col;
      AppState.corrSortDirection =
        col === 'rPhasic' || col === 'rTonic' || col === 'rPeaks'
          ? 'desc'
          : 'asc';
    }
    const cacheTarget =
  ...
  renderCorrelationTable(matrix, enrichedWalks, totalWalks) {
    ...
    if (noteEl) {
      const anyMeta = matrix.some(
        (r) =>
          r.mPhasic === 'meta' || r.mTonic === 'meta' || r.mPeaks === 'meta',
      );
      ...
      if (used === 1) {
        noteEl.innerHTML =
          notEnriched +
          'Single walk: <em>r</em> and <em>q</em> come from one recording, corrected for serial autocorrelation. Add walks to test whether an effect replicates.';
      } else if (anyMeta) {
        noteEl.innerHTML =
          notEnriched +
    ...
    const getInterpretation = (row) => {
      if (!row.hasVariance)
        return 'Not enough variation to measure — this factor barely changes along the route';
      const chans = [
        {
          q: row.qPhasic,
          p: row.pPhasic,
      ...
      if (sig.length > 0) {
        ...
        const how =
          top.m === 'meta'
            ? `consistent across your ${top.k} walks`
      ...
      if (best.m === 'meta') {
        ...
        const rawSig =
          typeof best.p === 'number' && isFinite(best.p) && best.p < 0.05;
        if (rawSig) {
          return (
            `Suggestive ${bestBand.label} link to ${dirWord(best.r)} ${best.name} ` +
    ...
    const displayMatrix = matrix.slice();
    ...
    displayMatrix.forEach((row) => {
      ...
      const cell = (r, q, m, k, i2, speedAdjR) => {
        const dir =
          Math.abs(r) < 0.1 ? 'dir-none' : r >= 0 ? 'dir-pos' : 'dir-neg';
        let chip;
        if (!row.hasVariance) {
          chip =
      ...
      const qCell = (q, m) =>
        row.hasVariance && isTested(m) && typeof q === 'number' && isFinite(q)
          ? formatP(q)
          : '—';
      tr.innerHTML = `
  ...
  drawRegressionScatterPlot(allData) {
    ...
    const isBinaryX = GSR_CONST.OSM_METRICS.some(
