...
export const MarchingSquares = {
  ...
  getContourLines(grid, rows, cols, bounds, isolevel) {
    ...
    const interpolate = (r1, c1, r2, c2) => {
      const v1 = grid[r1][c1];
      const v2 = grid[r2][c2];
      const lat1 = latGrid[r1],
        lon1 = lonGrid[c1];
      const lat2 = latGrid[r2],
        lon2 = lonGrid[c2];
      ...
      for (let c = 0; c < cols - 1; c++) {
        ...
        switch (cellIndex) {
          case 1:
            ...
            break;
          case 2:
            lines.push([getR(), getB()]);
            break;
          case 3:
            lines.push([getR(), getL()]);
            break;
          case 4:
            lines.push([getT(), getR()]);
            break;
          case 5: {
            const vCenter = MarchingSquares._saddleMean(
              vNW,
              vNE,
              vSE,
              vSW,
          ...
          case 11:
            lines.push([getR(), getT()]);
            break;
          case 12:
            lines.push([getL(), getR()]);
            break;
          case 13:
            lines.push([getB(), getR()]);
            break;
  ...
  getContourLinesMulti(grid, rows, cols, bounds, levels) {
    ...
    for (let r = 0; r < rows; r++)
      latGrid[r] = bounds.minLat + (r / rowsM1) * latSpan;
    for (let c = 0; c < cols; c++)
      lonGrid[c] = bounds.minLon + (c / colsM1) * lonSpan;
    ...
    for (let r = 0; r < rowsM1; r++) {
      for (let c = 0; c < colsM1; c++) {
        ...
        let cellMin = Infinity,
          cellMax = -Infinity;
        const hasNull = !okNW || !okNE || !okSE || !okSW;
        if (okNW) {
          if (vNW < cellMin) cellMin = vNW;
          if (vNW > cellMax) cellMax = vNW;
        }
        if (okNE) {
          if (vNE < cellMin) cellMin = vNE;
          if (vNE > cellMax) cellMax = vNE;
        }
        if (okSE) {
          if (vSE < cellMin) cellMin = vSE;
          if (vSE > cellMax) cellMax = vSE;
        }
        if (okSW) {
          if (vSW < cellMin) cellMin = vSW;
          if (vSW > cellMax) cellMax = vSW;
        }

        // Corner coordinates (looked up once per cell, shared across all levels).
        const latR = latGrid[r];
        const latR1 = latGrid[r + 1];
        const lonC = lonGrid[c];
        const lonC1 = lonGrid[c + 1];
        ...
        for (const isolevel of levels) {
          ...
          const cellIndex =
            (bNW ? 8 : 0) | (bNE ? 4 : 0) | (bSE ? 2 : 0) | (bSW ? 1 : 0);
          if (cellIndex === 0 || cellIndex === 15) continue;
