...
// regardless of the azimuth/aspect compass calibration. ─────────────────────
{
  ...
  const grid = Array.from({ length: rows }, () =>
    Array.from({ length: cols }, (_, c) => c * 3),
  );
  ...
  const oppositeShade = Hillshade.compute(grid, rows, cols, 1, 1, {
    azimuthDeg: oppositeAz,
    altitudeDeg,
  })[Math.floor(rows / 2) * cols + Math.floor(cols / 2)];
  assert(
    Math.abs(oppositeShade - minShade) < 0.02,
    `Shade directly opposite the brightest azimuth (${oppositeShade.toFixed(4)}) should equal the scanned minimum (${minShade.toFixed(4)})`,
  );
...
// formula's directional response is self-consistent under reflection. ──────
{
  const rows = 12,
    cols = 12;
  const eastRamp = Array.from({ length: rows }, () =>
    Array.from({ length: cols }, (_, c) => c * 3),
  );
  const westRamp = Array.from({ length: rows }, () =>
    Array.from({ length: cols }, (_, c) => (cols - 1 - c) * 3),
  );
  const altitudeDeg = 45;
  const midIdx = Math.floor(rows / 2) * cols + Math.floor(cols / 2);

  for (let az = 0; az < 360; az += 30) {
    const eastShade = Hillshade.compute(eastRamp, rows, cols, 1, 1, {
      azimuthDeg: az,
      altitudeDeg,
    })[midIdx];
    const oppositeShade = Hillshade.compute(westRamp, rows, cols, 1, 1, {
  ...
  console.log(
    '✓ Mirroring the slope direction rotates its illumination-vs-azimuth response by exactly 180°',
  );
}
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      if (grid[r][c] === null)
        assert(
          v === 0,
          `Masked cell (${r},${c}) left at 0 (caller never reads it, but shouldn't be garbage)`,
        );
    }
  }
  console.log(
    '✓ Masked cells handled without crashing; all shaded cells stay finite and within [0,1]',
  );
}
...
// fixture per the perf investigation that set this max), not hillshading. ──
{
  ...
  const grid = Array.from({ length: rows }, (_, r) =>
    Array.from(
      { length: cols },
      (_, c) => Math.sin(r * 0.3) + Math.cos(c * 0.2),
    ),
  );
  ...
  for (let i = 0; i < 2; i++)
    Hillshade.compute(grid, rows, cols, 1, 1, {
      azimuthDeg: 315,
      altitudeDeg: 45,
    });
  let minMs = Infinity;
  ...
  assert(
    minMs < 100,
    `400x400 hillshade pass completes in well under a frame budget (best of 5: ${minMs.toFixed(3)}ms)`,
  );
  console.log(
    `✓ 400x400 grid (UI max resolution) hillshades in ${minMs.toFixed(3)}ms (best of 5)`,
  );
}
...
// invisible in the convention-agnostic Tests 2/3 above. ────────────────────
{
  ...
  const southPeak = Array.from({ length: rows }, (_, r) =>
    new Array(cols).fill(-r * 3),
  );
  assert.strictEqual(
    argmaxAzimuth(southPeak),
    0,
    'A north-facing slope (aspect=0°) is brightest with the sun set to azimuthDeg=0 (true north)',
  );

  // Peak at the west (low c), downhill/aspect points east (90° true).
  const westPeak = Array.from({ length: rows }, () =>
    Array.from({ length: cols }, (_, c) => -c * 3),
  );
  assert.strictEqual(
    argmaxAzimuth(westPeak),
    90,
    'An east-facing slope (aspect=90°) is brightest with the sun set to azimuthDeg=90 (true east)',
  );
  ...
  const sePeak = Array.from({ length: rows }, (_, r) =>
    Array.from({ length: cols }, (_, c) => (c - r) * 3),
  );
  assert.strictEqual(
    argmaxAzimuth(sePeak),
    315,
    'A NW-facing slope (aspect=315°) is brightest with the sun set to azimuthDeg=315 (true NW) — the app default',
  );
  console.log(
    '✓ azimuthDeg is calibrated to true compass bearing (0=N, 90=E, 315=NW), not a raw math angle',
  );
}
...
// independently compute from the same inputs. ──────────────────────────────
{
  ...
  assert.strictEqual(
    ratioGrid[0][0],
    null,
    'Masked source cell stays null in the ratio grid',
  );
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      if (r === 0 && c === 0) continue;
      ...
      assert(
        Math.abs(ratioGrid[r][c] - expectedRatio) < 1e-9,
        `ratioGrid[${r}][${c}] matches the rank function's own output`,
      );
    }
  }
  ...
  assert(
    shade[1 * cols + 1] > 0,
    'An unmasked interior cell gets a real (non-zero-by-default) shade value',
  );
  console.log(
    "✓ shadeValueGrid() hillshades the same ratio field a caller's color fill uses, and exposes it via ratioGrid",
  );
}
...
// map_exporter.js, and buildRatioGrid instead of four independent copies. ──
{
  ...
  assert.strictEqual(
    Hillshade.valueRatio(
      ...
      50,
      sorted,
      (v, s) => s.indexOf(v) / (s.length - 1),
    ),
    0.5,
    'Uses rankFn when a real sorted-values distribution is given',
  );
  assert.strictEqual(
    Hillshade.valueRatio(30, 10, 50, undefined, undefined),
    0.5,
    'Falls back to linear min/max ratio when no rankFn/sortedVals is given (30 is halfway between 10 and 50)',
  );
  assert.strictEqual(
    Hillshade.valueRatio(5, 10, 10, undefined, undefined),
    0.5,
  ...
  console.log(
    '✓ valueRatio() is the single ratio formula: percentile rank when available, linear min/max fallback otherwise',
  );
}
...
// source grid's null mask. ──────────────────────────────────────────────────
{
  ...
  console.log(
    '✓ buildRatioGrid() applies valueRatio() per-cell and preserves the null mask',
  );
}
...
// change what gets rendered. ─────────────────────────────────────────────────
{
  ...
  const grid = Array.from({ length: rows }, (_, r) =>
    Array.from(
      { length: cols },
      (_, c) => Math.sin(r * 0.4) + Math.cos(c * 0.3),
    ),
  );
  const minVal = -2,
    maxVal = 2,
  ...
  const { shade: actual } = Hillshade.shadeValueGrid(grid, rows, cols, {
    minVal,
    maxVal,
    exaggeration,
    azimuthDeg,
  ...
  const heightGrid = ratioGrid.map((row) =>
    row.map((v) => (v === null ? null : v * exaggeration)),
  );
  const expected = Hillshade.compute(heightGrid, rows, cols, 1, 1, {
    azimuthDeg,
    altitudeDeg,
  });
  ...
  for (let i = 0; i < actual.length; i++) {
    assert(
      ...
      `Cell ${i}: zFactor-based shade (${actual[i]}) matches the old pre-multiplied-heightGrid shade (${expected[i]})`,
    );
  }
  console.log(
    "✓ shadeValueGrid()'s zFactor optimization produces numerically identical output to the old separate-heightGrid approach",
  );
}
