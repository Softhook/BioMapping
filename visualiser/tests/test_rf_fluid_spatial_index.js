...
const drawPoints = [
  { lat: 51.5, lon: -0.1, rssi_815: -70, rssi_868: -75, rssi_915: -60 },
  { lat: 51.51, lon: -0.09, rssi_815: -71, rssi_868: -76, rssi_915: -61 },
  { lat: 51.52, lon: -0.08, rssi_815: -72, rssi_868: -77, rssi_915: -62 },
  { lat: 51.53, lon: -0.07, rssi_815: -73, rssi_868: -78, rssi_915: -63 },
  { lat: 51.54, lon: -0.06, rssi_815: -74, rssi_868: -79, rssi_915: -64 },
  { lat: 51.55, lon: -0.05, rssi_815: -75, rssi_868: -80, rssi_915: -65 },
];

const ways = [];
// Cluster of buildings right around the first node.
for (let i = 0; i < 8; i++) {
  ways.push(buildingNear(51.5 + i * 0.00003, -0.1 + i * 0.00002));
}
// Cluster of buildings right around the last node.
for (let i = 0; i < 8; i++) {
  ways.push(buildingNear(51.55 + i * 0.00003, -0.05 + i * 0.00002));
}
const osmGeoms = { ways };
...
test('spatial grid index produces identical fanGeo output to a full linear scan', () => {
  ...
  assert.ok(
    gridNodes.every((n) => n.fanGeo.length === 24),
    'precondition: every node has a full ray fan',
  );
...
test('spatial grid index actually reduces candidate segments considered, not just a no-op passthrough', () => {
  ...
  assert.ok(
    totalCandidatesSeen < bruteForceUpperBound,
...
test('_buildSegmentGrid + _queryNearbySegments: candidate set (after the exact bbox filter) matches a brute-force scan for arbitrary query boxes', () => {
  ...
  for (let i = 0; i < 40; i++) {
    ...
    segments.push({
      p1: { lat, lon },
      p2: { lat: lat + 0.002, lon: lon + 0.002 },
    });
  }
  ...
  queryBoxes.forEach((bbox, idx) => {
    ...
    const indexed = gridFilter(bbox, idx + 1)
      .slice()
      .sort();
    assert.deepStrictEqual(
      new Set(indexed),
