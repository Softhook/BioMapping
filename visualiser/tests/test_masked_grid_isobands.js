...
console.log(
  '── Running Masked-Grid (Null-Cell Corridor) Isoband Regression Test ──',
);
...
// no level whose open path touches the mask silently comes back empty ──────
{
  ...
  const sortedVals = grid
    ...
    .sort((a, b) => a - b);
  assert(
    sortedVals.length > 0 && sortedVals.length < rows * cols,
    'Sanity: grid has both valid and masked (null) cells',
  );
  ...
  assert(
    levelsChecked >= 6,
    `Sanity: most of the ${contourCount} percentile levels actually had crossings to test (got ${levelsChecked})`,
  );
  console.log(
    `✓ All ${levelsChecked} contour levels with real crossings produce at least one fillable ring/closed path (no silently-missing bands)`,
  );
}
...
// the exact case reported — actually renders as substantial visible fill ───
{
  ...
  assert(
    segments.length > 0,
    'Low-value level has real crossings against the mask edge',
  );
  const stitched = GSRSpatialClustering.stitchSegments(segments);
  const { closed, open } = openClosedSplit(stitched);
  assert(
    closed.length > 0 || open.length > 0,
    'Low-value level produces closed or open paths',
  );
  const rings =
    open.length > 0
      ? ContourRingGeometry.closeOpenPaths(
          open,
          ContourRingGeometry.buildBoundaryLoops(grid, rows, cols, bounds),
          lowLevel,
        )
  ...
  assert(
    rings.length > 0,
    'Low-value band produces at least one fillable ring instead of vanishing',
  );
  ...
  console.log(
    `✓ Low-value ("green") band closes into a substantial, correctly-enclosed fill (not missing, not a tiny wrong sliver)`,
  );
}
...
// artifacts from noisy per-cell mask normals ────────────────────────────────
{
  ...
  assert(
    fills.length >= contours.length,
    `At least one filled shape per contour level with crossings (${fills.length} fills for ${contours.length} levels)`,
  );
  ...
  fills.forEach((p, i) => {
    ...
    assert(
      d.includes(' C'),
      `Fill ${i} uses smooth Bézier curves, not straight/blocky segments`,
    );
  ...
  console.log(
    `✓ All ${fills.length} masked-grid isoband fills render as smooth closed Bézier curves with no straight-line, spike, or sawtooth artifacts`,
  );
}
...
// of the valid-data region and needs no such nudge. ─────────────────────────
{
  ...
  const contourCount = 8;
  const level =
    sortedVals[Math.round((1 / (contourCount + 1)) * (sortedVals.length - 1))];
  ...
  assert(
    closed.length > 0 || open.length > 0,
    'Sanity: this level touches the mask boundary (closed or open path present)',
  );
  ...
  console.log(
    "✓ Outermost band's closed ring stays within the real mask coastline's own extent (no disconnected blob)",
  );
}
