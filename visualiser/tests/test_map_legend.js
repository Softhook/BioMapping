...
test('Map Legend: single mode - em_fog metric', async () => {
  ...
  const html = legendDiv.innerHTML;
  assert.ok(
    html.includes('EM Fog Index (0-100)'),
    'Should contain EM Fog Index title',
  );
...
test('Map Legend: single mode - OSM distance/canopy metrics use their own gradients', async () => {
  ...
  for (const [metric, start, end] of cases) {
    ...
    assert.ok(
      html.includes(start),
      `${metric} legend should use its start colour ${start}`,
    );
    assert.ok(
      html.includes(end),
      `${metric} legend should use its end colour ${end}`,
    );
  }
});
...
test('GSRUI.drawRegressionScatterPlot: data source resolution matches viewMode', async () => {
  ...
  assert.strictEqual(
    lastX.length,
    3,
    'Should resolve to collective-track cache',
  );
...
test('GSRUI._percentileSorted: robust axis-clip bounds used by the regression scatter', async () => {
  ...
  assert.strictEqual(
    PS([10, 20, 30, 40], 0.5),
    25,
    'sorted median by interpolation',
  );
});
...
test('GSRUI.drawRegressionScatter: continuous X plots points + trend + badge, binary X draws per-group boxes, empty data plots nothing (fake 2D context)', async () => {
  ...
  const xCont = Array.from(
    { length: 500 },
    (_, i) => (i % 97) + (i > 480 ? 5000 : 0),
  ); // + a few spikes
  const yCont = Array.from(
    { length: 500 },
    (_, i) => Math.sin(i / 11) + (i % 5) * 0.2,
  );
  const contOps = opsFor(() =>
    window.GSRUI.drawRegressionScatter(
      canvas,
      xCont,
      yCont,
      0.01,
  ...
  assert.ok(
    contOps.includes('arc'),
    'continuous path plots point markers (ctx.arc)',
  );
  assert.ok(
    contOps.includes('fillText'),
    'continuous path draws axis labels / R² badge (ctx.fillText)',
  );
  assert.ok(
    contOps.includes('stroke') || contOps.includes('lineTo'),
    'continuous path draws the OLS trend line',
  );

  const xBin = Array.from({ length: 400 }, (_, i) => (i % 3 === 0 ? 1 : 0));
  const yBin = Array.from({ length: 400 }, (_, i) =>
    i % 3 === 0 ? 2 + (i % 7) * 0.1 : 1 + (i % 7) * 0.1,
  );
  const binOps = opsFor(() =>
    window.GSRUI.drawRegressionScatter(
      canvas,
  ...
  assert.ok(
    binOps.filter((o) => o === 'rect' || o === 'fillRect' || o === 'strokeRect')
      .length >= 2,
    'binary path draws a box-and-whisker box per group (>= 2 rect ops)',
  );
  assert.ok(
    binOps.includes('fillText'),
    'binary path still draws the point-biserial r badge',
  );
  ...
  assert.ok(
    !emptyOps.includes('arc'),
    'empty data path plots no point markers',
  );
});
