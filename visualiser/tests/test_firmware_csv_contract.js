...
if (!fs.existsSync(FW)) {
  test('firmware ⇄ visualiser save-protocol contract', {
    skip: 'firmware/ not present in this checkout',
  }, () => {});
} else {
  runContractTests();
}
...
function runContractTests() {
  ...
  const readLive = (name) => {
    ...
    return fs.readFileSync(
      name.endsWith('.js') && !fs.existsSync(full) && fs.existsSync(mjsFull)
        ? mjsFull
        : full,
      'utf8',
    );
  ...
  function specFrag(spec) {
    ...
    if (f) return `-?\\d+\\.\\d{${f[1]}}`;
    throw new Error(
      `SAVE-PROTOCOL CONTRACT: unhandled printf spec "${spec}" — extend specFrag().`,
    );
  }
  ...
  test('CSV: the integrity marker is verbatim firmware SD_LOGGER_INTEGRITY_LINE', () => {
    ...
    const fwMarker = cStr(
      locate(
        sd,
        /#define\s+SD_LOGGER_INTEGRITY_LINE\s+("(?:[^"\\]|\\.)*")/,
  ...
  test('CSV: the GPS+GSR column header equals firmware BIOMAP_CSV_COLS_GPS_GSR_PROD', () => {
    ...
    if (!line) {
      assert.fail(
        'SAVE-PROTOCOL CONTRACT: BIOMAP_CSV_COLS_GPS_GSR_PROD not found in ' +
  ...
  test("CSV: a valid-fix row matches biomap_format_gps_row()'s printf layout", () => {
    ...
    assert.strictEqual(
      fwSpecs.length,
      11,
      'firmware valid-fix row is 11 columns',
    );
  ...
  test("CSV: a no-fix row matches biomap_format_gps_row()'s empty-GPS-columns branch", () => {
    ...
    const fwSpecs = cStr(
      locate(
        fn,
        /snprintf\([^,]+,[^,]+,\s*("%\.2f,,+[^"]*")/,
        'the no-fix snprintf format in biomap_format_gps_row()',
      )[1],
    ).split(',');
  ...
  test('CSV: the "# End" trailer token order matches sd_logger_write_trailer()', () => {
    ...
    const endFmt = cStr(
      locate(
        sd,
        /("# End rows:[^"]*\\n")/,
        'the "# End" trailer format in modules/sd_logger.c',
      )[1],
    ).trimEnd();
    ...
    assert.deepStrictEqual(
      fwTokens,
      ['rows', 'bytes', 'crc32', 'end_time', 'overflows', 'flush_fails'],
    ...
    assert.ok(
      /crc32:%08lx/.test(endFmt),
      'firmware crc32 token is still %08lx (lower hex, width 8)',
    );
    ...
    const trailer = buildLiveCsv(
      ...
      .pop();
    const liveTokens = [
      ...trailer.replace(/^# End\s+/, '').matchAll(/([a-z0-9_]+):/g),
    ].map((m) => m[1]);
    assert.deepStrictEqual(
      liveTokens,
      fwTokens,
  ...
  test('WIRE: live_binary_parser.js field offsets + size match firmware bt_stream.{c,h}', () => {
    ...
    const fwSize = Number(
      locate(
        bth,
        /#define\s+BT_STREAM_PACKET_SIZE\s+(\d+)/,
        'BT_STREAM_PACKET_SIZE in modules/bt_stream.h',
      )[1],
    );
    const jsSize = Number(
      locate(
        parser,
        /\bPACKET_SIZE\s*=\s*(\d+)/,
        'PACKET_SIZE in live_binary_parser.js',
      )[1],
    );
    ...
    const fwMagic = [];
    for (const m of btc.matchAll(/out\[([01])\]\s*=\s*0x([0-9a-fA-F]+)/g))
      fwMagic[Number(m[1])] = parseInt(m[2], 16);
    const jsMagic = [
      Number(
        locate(
          parser,
          /MAGIC_0\s*=\s*(0x[0-9a-fA-F]+)/,
          'MAGIC_0 in live_binary_parser.js',
        )[1],
      ),
      Number(
        locate(
          parser,
          /MAGIC_1\s*=\s*(0x[0-9a-fA-F]+)/,
          'MAGIC_1 in live_binary_parser.js',
        )[1],
      ),
    ];
    ...
    for (const m of btc.matchAll(/memcpy\(out\s*\+\s*(\d+)\s*,/g))
      fwOffsets.add(Number(m[1]));
    for (const m of btc.matchAll(/\bout\[(\d+)\]\s*=/g))
      fwOffsets.add(Number(m[1]));
    fwOffsets.delete(0);
    fwOffsets.delete(1);
    ...
    for (const f of jsFields) {
      assert.ok(
        f.w,
        `live_binary_parser.js uses an unmapped DataView getter at offset ${f.off}`,
      );
    }
    ...
    assert.strictEqual(
      cursor,
      jsSize,
      'live_binary_parser.js fields do not fill the packet exactly',
    );
  });
}
