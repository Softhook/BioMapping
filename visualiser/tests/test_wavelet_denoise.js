/**
 * Unit tests for the DWT wavelet library (visualiser/src/signal/dwt_filter.mjs):
...
function noisySCRTrace(n, Fs, noiseAmp, seed) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const b =
      0.5 +
      scr(t, (n * 0.25) / Fs, 2.5, 0.8, 2.5) +
      scr(t, (n * 0.6) / Fs, 3.0, 0.9, 2.0);
    clean.push(b);
...
test('reconstructFull: decompose → reconstructFull recovers the signal to ~1e-9', () => {
  for (const n of [512, 777, 2000, 9000]) {
    const x = Array.from(
      { length: n },
      (_, i) => 3 + 0.001 * i + 0.7 * Math.sin(i / 9) + 0.2 * Math.sin(i / 2.3),
    );
    for (const L of [3, 4, 5]) {
      ...
      assert.ok(
        mx < 1e-9,
        `n=${n} L=${L}: round-trip error ${mx.toExponential(2)}`,
      );
    }
  }
});
...
test('denoise: length preserved for even, odd, near-2^levels, and long inputs', () => {
  for (const n of [400, 401, 15, 16, 17, 9000]) {
    const x = Array.from(
      { length: n },
      (_, i) => Math.sin(i / 5) + (i % 3) * 0.1,
    );
    assert.strictEqual(
      DWT.denoise(x, { sampleRate: 10, mode: 'light', shifts: 4 }).length,
      n,
    );
  }
});
...
test('denoise: reduces broadband white-noise variance (light < 0.6x, strong < 0.4x)', () => {
  ...
  const vL = variance(
    DWT.denoise(noisy, { sampleRate: 10, mode: 'light', shifts: 8 }).map(
      (v, i) => v - clean[i],
    ),
  );
  const vS = variance(
    DWT.denoise(noisy, { sampleRate: 10, mode: 'strong', shifts: 16 }).map(
      (v, i) => v - clean[i],
    ),
  );
  assert.ok(
    vL < 0.6 * vIn,
    `light residual variance ${(vL / vIn).toFixed(2)}x`,
  );
  assert.ok(
    vS < 0.4 * vIn,
    `strong residual variance ${(vS / vIn).toFixed(2)}x`,
  );
...
test('denoise: SCR peak amplitude is retained (light within 5%, strong within 8%)', () => {
  const { clean, noisy } = noisySCRTrace(4000, 10, 0.1, 7);
  const ci = apexIdx(clean);
  const cAmp = clean[ci] - 0.5;
  for (const [mode, tol, K] of [
    ['light', 0.05, 8],
    ['strong', 0.08, 16],
  ]) {
...
test('denoise: SCR 10-90% rise time is not badly inflated (< 60% vs clean)', () => {
  ...
  for (const mode of ['light', 'strong']) {
    ...
    assert.ok(
      ro < rc * 1.6,
      `${mode}: rise ${ro.toFixed(2)}s vs clean ${rc.toFixed(2)}s`,
    );
  }
});
...
test('denoise: a lone spike is left largely intact with no ringing (it is not a spike remover)', () => {
  ...
  assert.ok(
    out[i0] - base > 0.4,
    `spike should survive detail-band shrinkage: ${(out[i0] - base).toFixed(2)}`,
  );
  for (let k = 4; k <= 12; k++) {
    assert.ok(
      Math.abs(out[i0 - k] - spiked[i0 - k]) < 0.2,
      `ringing at neighbour -${k}`,
    );
    assert.ok(
      Math.abs(out[i0 + k] - spiked[i0 + k]) < 0.2,
      `ringing at neighbour +${k}`,
...
test('denoise: loose idempotence — a second pass changes little', () => {
  ...
  for (let i = 0; i < x.length; i++)
    mx = Math.max(mx, Math.abs(twice[i] - once[i]));
