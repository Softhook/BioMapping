...
console.log(
  `  Duration: ${durationSec.toFixed(0)}s (${durationMin.toFixed(1)} min)`,
);
console.log(`  Samples:  ${off.raw.length} at ${off.sampleRate} Hz`);
console.log(
  `  Peaks:    ${off.peaks.length} (${(off.peaks.length / durationMin).toFixed(1)}/min)`,
);

assert(off.peaks.length > 0, 'Non-deconvolution finds peaks on track 059');
assertEq(
  off.phasicDriver.length,
  0,
  'Non-deconvolution leaves phasicDriver empty',
);
assertEq(
  off.phasicClean.length,
  0,
...
console.log(
  `  Detected peaks:  ${on.peaks.length} (${(on.peaks.length / durationMin).toFixed(1)}/min)`,
);
...
// correction (density is high on this busy track), but not full forcing.
{
  ...
  console.log(
    `    ratio          = ${ratio.toFixed(6)} (want 1.0-1.3: partial correction on a busy real track, not forced to exactly 1.0, not left at the uncorrected ~1.63)`,
  );
  assert(
...
{
  const violations = on.peaks.filter(
    (p) => p.amplitude < BASE_PARAMS.peakThreshold,
  );
  assertEq(
    violations.length,
    0,
...
// ── 7. All peak fields are finite and in valid ranges ────────────────────────
{
  ...
  for (const p of on.peaks) {
    if (!isFinite(p.amplitude) || p.amplitude <= 0) badAmp++;
...
// a different array (e.g. phasicOrig before the decon pipeline ran).
{
  ...
  let mismatches = 0;
  for (const p of on.peaks) {
    if (Math.abs(p.onsetValue - (cleanVals[p.onsetIndex] || 0)) > 1e-9)
      mismatches++;
  }
  assertEq(
    mismatches,
...
// (MEMORABLE_EVENTS.MIN_SEPARATION_M) can pull the realised count below that.
{
  ...
  assert(
    on.memorableEvents.length > 0 && on.memorableEvents.length <= targetCount,
...
// stale driver/clean/phasicOrig state left behind.
{
  ...
  tog.analyze({ ...BASE_PARAMS, useDeconvolution: false });
  assertEq(
    tog.phasicDriver.length,
    0,
    'Toggle off clears phasicDriver on track 059',
  );
  assertEq(
    tog.phasicClean.length,
    0,
    'Toggle off clears phasicClean on track 059',
  );
  assertEq(tog._phasicOrig, null, 'Toggle off clears _phasicOrig on track 059');
  assertEq(
    tog.peaks.length,
    off.peaks.length,
...
// that could make results differ between calls.
{
  ...
  const a1 = new GSRAnalyzer();
  a1.parseCSV(csvText);
  a1.analyze({ ...BASE_PARAMS, useDeconvolution: true });
  const a2 = new GSRAnalyzer();
  a2.parseCSV(csvText);
  a2.analyze({ ...BASE_PARAMS, useDeconvolution: true });
  const sig1 = a1.peaks
...
console.log(`\n${'='.repeat(60)}`);
console.log(
  `Track 059 deconvolution regression suite: ${passed} passed, ${failed} failed`,
);
