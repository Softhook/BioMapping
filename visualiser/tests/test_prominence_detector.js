...
assert(
  prom.peaks.every(
    (p) => typeof p.prominence === 'number' && p.prominence >= THR - 1e-9,
  ),
...
const isolatedBig = trough.peaks.filter(
  (p, k) =>
    p.amplitude >= 0.3 &&
    (k === 0 || p.time - tT[k - 1] >= 3) &&
...
const keptBig = isolatedBig.filter((p) =>
  pT.some((t) => Math.abs(t - p.time) <= GAP),
).length;
assert(
  isolatedBig.length > 0 && keptBig === isolatedBig.length,
...
const tightShape = analyze({
  ...
  shapeMinRiseTime: 1.0,
  shapeMaxRiseTime: 3.0,
  shapeMaxHalfRecovery: 4.0,
  shapeMaxSkewRatio: 0.8,
  shapeMinSnr: 4.0,
});
assert(
  tightShape.peaks.length === prom.peaks.length,
...
assert(
  highQ.peaks.length < prom.peaks.length,
  `Min Peak Quality does gate (${highQ.peaks.length} < ${prom.peaks.length})`,
);
assert(
  highQ.peaks.every((p) => p.qualityScore >= 0.7),
  'and it is actually applied per peak',
);
...
assert(
  keys.every((k) => k in prom.peaks[0]),
  'peak objects carry the full field set + prominence',
);

// Structural invariants.
const pv = prom.phasic.map((d) => d.val);
assert(
  prom.peaks.every((p) => p.onsetIndex <= p.index),
  'onset index never after the apex',
);
assert(
  prom.peaks.every((p) => p.recoveryIndex === -1 || p.recoveryIndex > p.index),
  'recovery index, when found, after the apex',
);
assert(
  prom.peaks.every((p) => p.amplitude > 0),
  'all amplitudes positive',
);
assert(
  prom.peaks.every((p) => Number.isFinite(p.time) && Number.isFinite(p.value)),
  'time/value finite',
);
assert(
  prom.peaks.every(
...
for (let i = 1; i < pT.length; i++)
  minGap = Math.min(minGap, prom.peaks[i].time - prom.peaks[i - 1].time);
assert(
  minGap >= GAP - 1e-6,
  `no two peaks closer than PEAK_MIN_GAP (min ${minGap.toFixed(3)}s)`,
);
...
// Deconvolution state untouched; precedence over deconvolution.
assert(
  prom.phasicDriver.length === 0 &&
    prom.phasicClean.length === 0 &&
    prom.phasicDriverPeaks.length === 0,
  'prominence path leaves deconvolution state empty',
);
assert(
  prom._phasicOrig === null,
  'prominence path clears any stale pre-deconvolution phasic backup',
);
const both = analyze({ usePeakProminence: true, useDeconvolution: true });
assert(
  both.phasicDriverPeaks.length === 0,
...
{
  const demoCsv = fs.readFileSync(
    path.join(__dirname, '../fixtures/default_processed.csv'),
    'utf8',
  );
  const off = new GSRAnalyzer();
  off.parseCSV(demoCsv);
  off.analyze({ ...D, usePeakProminence: false }, 0);
  const on = new GSRAnalyzer();
  on.parseCSV(demoCsv);
  on.analyze({ ...D, usePeakProminence: true }, 0);

  const biggestOff = off.peaks.reduce((a, b) =>
  ...
  assert(
    p1 != null,
    `P1 (t=${biggestOff.time.toFixed(1)}s) is detected in prominence mode`,
  );
  assert(
  ...
  assert(
    p1 && on.memorableEvents.includes(p1),
  ...
  const mostProm = on.peaks
    .filter((p) => !p.excluded)
    .reduce((a, b) => (b.prominence > a.prominence ? b : a));
  assert(
    on.memorableEvents[0] &&
      on.memorableEvents[0].prominence >= mostProm.prominence - 1e-9,
...
//    walked through; one above it is not. ──
{
  ...
  assert(
    a._findOnsetIndex(mk(THR * 0.5), 23, 50) === 21,
...
// double-count the shared rise.
{
  ...
  const burst = t.peaks
    .filter((p) => p.time > 237 && p.time < 241)
    .sort((a, b) => a.time - b.time);
  if (burst.length >= 2) {
    assert(
      burst[1].onsetIndex > burst[0].index - 3,
...
assert(
  Array.isArray(tiny.peaks) && tiny.peaks.length === 0,
  'n<3 signal yields an empty peak list, no throw',
);
