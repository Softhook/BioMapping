...
console.log(
  `  ${off.peaks.length} peaks, duration ${(off.raw[off.raw.length - 1].time - off.raw[0].time).toFixed(0)}s`,
);
assert(off.peaks.length > 0, 'Non-deconvolution finds peaks on track 053');
assert(
  off.phasicDriver.length === 0 && off.phasicClean.length === 0,
  'Non-deconvolution leaves driver/clean state empty',
);

console.log('\n── Deconvolution pipeline ──');
const on = analyzeTrack(true);
console.log(
  `  ${on.phasicDriverPeaks.length} driver impulses -> ${on.peaks.length} detected peaks`,
);
console.log(
  `  Rate: ${(on.peaks.length / ((on.raw[on.raw.length - 1].time - on.raw[0].time) / 60)).toFixed(2)} peaks/min`,
);

assert(
  !on.phasicDeconvTruncated,
  'Matching pursuit converges within maxIter on track 053 (not truncated)',
);
assertEq(
  on.phasicDriver.length,
  off.raw.length,
  'phasicDriver populated at full track length',
);
assertEq(
  on.phasicClean.length,
  off.raw.length,
...
assert(
  rate > 1 && rate < 40,
  `Peak rate is physiologically plausible: ${rate.toFixed(2)}/min (not the ~92/min the original per-window bug produced)`,
);

// No amplitude-threshold violations.
const belowThreshold = on.peaks.filter((p) => p.amplitude < 0.02);
assertEq(
  belowThreshold.length,
  0,
  `All peaks respect peakThreshold (${belowThreshold.length} violations)`,
);
...
for (let i = 1; i < sortedTimes.length; i++)
  minGap = Math.min(minGap, sortedTimes[i] - sortedTimes[i - 1]);
assert(
  minGap >= global.GSR_CONST.SCRF.minImpulseGapSec - 1e-9,
...
// trueMax(phasicOrig) > p.value would flag correct behaviour as a failure.
{
  ...
  for (const p of on.peaks) {
    const lo = Math.max(0, p.index - halfWin),
      hi = Math.min(cleanVals.length - 1, p.index + halfWin);
    let trueMax = -Infinity;
    for (let i = lo; i <= hi; i++)
      if (cleanVals[i] > trueMax) trueMax = cleanVals[i];
    ...
    if (trueMax > p.value + diffThreshold) {
      offApex++;
      maxDiff = Math.max(maxDiff, trueMax - p.value);
    }
  }
  const offApexRate = offApex / on.peaks.length;
  console.log(
    `  Peak markers off true local max: ${offApex}/${on.peaks.length} (${(offApexRate * 100).toFixed(1)}%), max diff ${maxDiff.toFixed(4)}uS`,
  );
  ...
  assert(
    offApexRate < 0.15,
    `Peak markers mostly land at true local maxima: ${(offApexRate * 100).toFixed(1)}% off (want <15%)`,
  );
}
...
// raw noisy signal instead.
{
  ...
  assertEq(
    mismatches,
    0,
    'onsetValue matches the reconstructed curve value at onsetIndex (not hardcoded 0)',
  );
}
...
// pre-fix level will still be caught immediately.
{
  ...
  for (const p of isolated) {
    ...
    for (const q of on.peaks)
      bestDist = Math.min(bestDist, Math.abs(q.time - p.time));
    if (bestDist <= 1.5) matched++;
  }
  const rate = isolated.length > 0 ? matched / isolated.length : 1;
  console.log(
    `  Agreement on isolated (unambiguous) peaks: ${matched}/${isolated.length} (${(rate * 100).toFixed(1)}%)`,
  );
  assert(
    isolated.length >= 20,
    `Enough isolated peaks on track 053 to make the agreement check meaningful (${isolated.length})`,
  );
  ...
  assert(
    rate >= 0.7,
    `Deconvolution agrees with the default detector on >=70% of unambiguous isolated peaks (got ${(rate * 100).toFixed(1)}%)`,
  );
}
...
// and no two hotspots closer than MEMORABLE_EVENTS.MIN_SEPARATION_M.
{
  ...
  assertEq(
    badField,
    false,
    'Every peak on track 053 has a valid salienceScore in [0,1]',
  );
  assert(
    on.memorableEvents.length > 0,
    'Track 053 (a busy, eventful real recording) has at least one memorable event',
  );
  ...
  assert(
    on.memorableEvents.length <= targetCount,
    `memorableEvents is at most the top 2% of peaks (target ${targetCount}, got ${on.memorableEvents.length})`,
  );
  const allValid = on.memorableEvents.every(
    (p) => on.peaks.includes(p) && !p.excluded,
  );
  ...
  for (let i = 1; i < on.memorableEvents.length; i++) {
    if (
      on.memorableEvents[i].amplitude >
      on.memorableEvents[i - 1].amplitude + 1e-9
    ) {
  ...
  assert(
    coords.length < 2 || minPair >= minSep - 1e-6,
...
// Toggle-sequence state leakage guard, specifically on real track data.
{
  ...
  toggler.analyze(paramsOff);
  assertEq(
    toggler.phasicDriver.length,
    0,
    'Toggling back off clears phasicDriver on real track data',
  );
  assertEq(
    toggler.phasicClean.length,
    0,
    'Toggling back off clears phasicClean on real track data',
  );
  assertEq(
    toggler._phasicOrig,
    null,
    'Toggling back off clears _phasicOrig on real track data',
  );
  assertEq(
    toggler.peaks.length,
    off.peaks.length,
    'Toggling back off reproduces the exact non-deconvolution peak count',
  );
}

console.log(`\n${'='.repeat(60)}`);
console.log(
  `Track 053 deconvolution regression suite: ${passed} passed, ${failed} failed`,
);
