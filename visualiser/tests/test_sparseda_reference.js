...
const ref = JSON.parse(
  fs.readFileSync(
    path.join(__dirname, 'fixtures/sparseda_reference.json'),
    'utf8',
  ),
);
...
test('SparsEDA matches the reference implementation on an 8 Hz fixture with solver-aligned parameters', () => {
  const res = SCRDeconvolution.deconvolve(
    Float64Array.from(ref.signal),
    ref.sr,
    {
      algorithm: 'sparseda',
  ...
  assert.strictEqual(
    res.applyRescale,
    false,
    'reference SparsEDA path should not use matching-pursuit rescaling',
  );
  ...
  assert.ok(
    driverErr < 1e-10,
    `driver relRMSE should be < 1e-10, got ${driverErr.toExponential(2)}`,
  );
  assert.ok(
    tonicErr < 1e-10,
    `tonic relRMSE should be < 1e-10, got ${tonicErr.toExponential(2)}`,
  );
  assert.ok(
    mseErr < 1e-10,
    `mse relRMSE should be < 1e-10, got ${mseErr.toExponential(2)}`,
  );

  const refNonzero = ref.driver.reduce((n, v) => n + (v > 0 ? 1 : 0), 0);
  const jsNonzero = Array.from(res.driver).reduce(
    (n, v) => n + (v > 0 ? 1 : 0),
    0,
  );
  assert.strictEqual(
    jsNonzero,
    refNonzero,
    'driver sparsity pattern should match the reference',
  );
});
...
test('SparsEDA keeps original array length when internally resampling 10 Hz input to the 8 Hz reference rate', () => {
  ...
  assert.ok(
    Array.from(res.driver).some((v) => v > 0),
    'resampled 10 Hz input should still produce sparse driver events',
  );
});
...
test('SparsEDA treats a positive flat signal as a zero-driver solution', () => {
  ...
  assert.strictEqual(
    res.iterations,
    0,
    'flat signal should not activate the dictionary',
  );
  assert.ok(
    Array.from(res.driver).every((v) => v === 0),
    'flat signal should produce no sparse driver',
  );
});
...
test('SparsEDA pruning keeps clean consistent with the kept driver', () => {
  ...
  assert.strictEqual(
    nonzeroDriver,
    0,
    'sanity check: rho=2 should prune every driver event',
  );
  assert.strictEqual(
    maxClean,
    0,
    'clean reconstruction should also drop to zero when no driver event is kept',
  );
});

test('SparsEDA epsilon remains an active stop threshold', () => {
  const strict = SCRDeconvolution.deconvolve(
    Float64Array.from(ref.signal),
    ref.sr,
    {
      algorithm: 'sparseda',
  ...
  const loose = SCRDeconvolution.deconvolve(
    Float64Array.from(ref.signal),
    ref.sr,
    {
      algorithm: 'sparseda',
  ...
  assert.ok(
    ...
    `looser epsilon should stop earlier (${loose.iterations} < ${strict.iterations})`,
  );
});
