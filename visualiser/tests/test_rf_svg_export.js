...
assert(
  renderer.cachedNodes.length > 0,
  'RFFluidRenderer should cache RF nodes',
);
...
assert(
  Array.isArray(svgElements.defs),
  'exportToSvgElements should return defs array',
);
assert(
  Array.isArray(svgElements.polygons),
  'exportToSvgElements should return polygons array',
);
assert(
  svgElements.polygons.length > 0,
  'Should generate SVG polygon elements for RF ray fans',
);

const hasGradient = svgElements.defs.some((d) => d.includes('<radialGradient'));
const hasBuildingMask = svgElements.defs.some((d) =>
  d.includes('id="rfBuildingMask"'),
);

assert(
  hasGradient,
  'SVG defs must contain radialGradient definitions for RF nodes',
);
assert(
  hasBuildingMask,
  'SVG defs must contain building clip mask for OSM shapes',
);

console.log(
  `✔ Generated ${svgElements.polygons.length} RF vector polygon elements`,
);
console.log(
  `✔ Generated ${svgElements.defs.length} SVG defs (radial gradients + building mask)`,
);
...
assert(
  rfLayerData.polygons.length > 0,
  '_rfFluid must collect RF polygons from renderer',
);
...
assert(
  fullSvgMarkup.includes('id="RF_Fluid_Field"'),
  'SVG output must include RF_Fluid_Field group layer',
);
assert(
  fullSvgMarkup.includes('id="RF_815MHz_LTE"'),
  'SVG output must include RF_815MHz_LTE sub-layer',
);
assert(
  fullSvgMarkup.includes('id="RF_868MHz_Grid"'),
  'SVG output must include RF_868MHz_Grid sub-layer',
);
assert(
  fullSvgMarkup.includes('id="RF_915MHz_SubGHz"'),
  'SVG output must include RF_915MHz_SubGHz sub-layer',
);
assert(
  fullSvgMarkup.includes('mask="url(#rfBuildingMask)"'),
  'RF_Fluid_Field layer must apply building mask',
);
assert(
  !fullSvgMarkup.includes('NaN'),
  'SVG output must contain zero NaN values',
);

console.log(
  '✔ Generated separated Illustrator frequency sub-layers (815, 868, 915 MHz)',
);
console.log(
  '✔ Full SVG rendering integration passed with zero NaN coordinates',
);
...
assert(
  rfFieldTagMatch,
  'RF_Fluid_Field opening tag must be present in the SVG output',
);
const rfFieldTag = rfFieldTagMatch[0];
assert(
  rfFieldTag.includes('mask="url(#rfBuildingMask)"'),
  'RF_Fluid_Field must still carry the building mask when one exists',
);
assert(
  rfFieldTag.includes('isolation: isolate'),
  'RF_Fluid_Field must isolate its blend context so its screen blend does not reach through to the tiles below it',
);
assert(
  !rfFieldTag.includes('mix-blend-mode'),
  'RF_Fluid_Field itself must NOT carry mix-blend-mode — only its per-band sub-layers should (screening against the tiles is exactly the bug)',
);

console.log(
  '✔ RF_Fluid_Field isolates its blend context from the tiles below it, and still keeps the building mask',
);
