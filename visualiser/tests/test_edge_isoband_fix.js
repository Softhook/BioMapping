...
assert(
  segments.length > 0,
  'Marching squares finds a crossing for the edge-pinned hot region',
);
...
assert(
  anyOpen,
  'Sanity check: this edge-pinned region really does stitch into an OPEN path (the pre-fix failure mode)',
);
console.log(
  '✓ Confirmed the underlying isoline for an edge-pinned region is open (not self-closing)',
);
...
const fillPaths = layers.isobands.filter(
  (p) => /fill="#[0-9a-f]{6}"/i.test(p) && !/fill="none"/i.test(p),
);
assert(
  fillPaths.length > 0,
  'Vector_Surface_Isobands contains at least one FILLED polygon for the edge-pinned region',
);
...
fillPaths.forEach((p) => {
  ...
  assert(
    dMatch[1].trim().endsWith('Z'),
    `Filled isoband path is a closed loop (d ends with Z): ${dMatch[1]}`,
  );
});
console.log(
  `✓ ${fillPaths.length} filled, closed isoband polygon(s) generated for the region touching the grid edge`,
);
