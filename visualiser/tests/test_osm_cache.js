...
const bboxA = { minLat: 51.5, minLon: -0.12, maxLat: 51.51, maxLon: -0.1 }; // outer/broad
const bboxB = {
  minLat: 51.503,
  minLon: -0.115,
  maxLat: 51.507,
  maxLon: -0.105,
}; // inner, fully inside A
const bboxC = { minLat: 51.6, minLon: -0.2, maxLat: 51.61, maxLon: -0.19 }; // disjoint from A
...
{
  const partial = {
    minLat: bboxA.minLat + 0.005,
    minLon: bboxA.minLon + 0.005,
...
{
  const nearlyA = {
    minLat: bboxA.minLat + 0.0001,
    minLon: bboxA.minLon + 0.0001,
    maxLat: bboxA.maxLat - 0.0001,
...
// alongside the old one (some duplication between the two is expected).
{
  ...
  assertEq(
    match,
    null,
    '_pickBestMatch — partially-overlapping request is a miss (no merge/union support)',
  );
}
...
// 2c. Prefers the tightest-fitting (smallest-area) containing entry
{
  ...
  assertEq(
    match.data,
    'tight',
    '_pickBestMatch — prefers smallest containing entry over broader one',
  );
}
...
{
  const entries = [
    { bbox: bboxA, queryVersion: 0, fetchedAt: now, data: 'old-query-shape' },
  ];
  const match = OsmCache._pickBestMatch(entries, bboxB, 1, now);
...
// 2e. Ignores expired entries (older than CACHE_TTL_MS)
{
  const stale = {
    bbox: bboxA,
    queryVersion: 1,
    fetchedAt: now - OsmCache.CACHE_TTL_MS - 1000,
    data: 'stale',
  };
...
// 3a. _bboxIntersects
assert(
  OsmCache._bboxIntersects(bboxA, bboxPartial),
  '_bboxIntersects — partially-overlapping bboxes → true',
);
assert(
  OsmCache._bboxIntersects(bboxA, bboxB),
  '_bboxIntersects — full containment counts as intersecting → true',
);
assert(
  OsmCache._bboxIntersects(bboxA, bboxA),
  '_bboxIntersects — bbox intersects itself → true',
);
assert(
  !OsmCache._bboxIntersects(bboxA, bboxC),
  '_bboxIntersects — disjoint bboxes → false',
);
...
// 3b. _unionBBox
{
  ...
  assertEq(
    u.minLat,
    Math.min(bboxA.minLat, bboxPartial.minLat),
    '_unionBBox — minLat is the min of inputs',
  );
  assertEq(
    u.maxLat,
    Math.max(bboxA.maxLat, bboxPartial.maxLat),
    '_unionBBox — maxLat is the max of inputs',
  );
  assertEq(
    u.minLon,
    Math.min(bboxA.minLon, bboxPartial.minLon),
    '_unionBBox — minLon is the min of inputs',
  );
  assertEq(
    u.maxLon,
    Math.max(bboxA.maxLon, bboxPartial.maxLon),
    '_unionBBox — maxLon is the max of inputs',
  );
}
{
  ...
  assertEq(
    u.minLat,
    bboxA.minLat,
    '_unionBBox — single-bbox union equals that bbox (minLat)',
  );
  assertEq(
    u.maxLon,
    bboxA.maxLon,
    '_unionBBox — single-bbox union equals that bbox (maxLon)',
  );
}

// 3c. _bboxAreaKm2 — same known-answer case as OSMEnricher.calculateBBoxAreaKm2
{
  const area = OsmCache._bboxAreaKm2({
    minLat: 0,
    maxLat: 0.01,
    minLon: 0,
    maxLon: 0.01,
  ...
  assertClose(
    ...
    1.239,
    0.02,
    '_bboxAreaKm2 ≈ 1.24 km² for 0.01°×0.01° at equator',
  );
}
...
// 3d. _findOverlapping — finds partial overlaps, excludes disjoint entries
{
  ...
  assertEq(
    overlapping.length,
    1,
    '_findOverlapping — finds exactly the one overlapping entry',
  );
  assertEq(
    overlapping[0].id,
    1,
    '_findOverlapping — excludes the disjoint entry',
  );
}
...
{
  const wrongVersion = [
    { id: 1, bbox: bboxA, queryVersion: 0, fetchedAt: now, data: 'x' },
  ];
  assertEq(
    OsmCache._findOverlapping(wrongVersion, bboxPartial, 1, now).length,
    0,
  ...
  assertEq(
    OsmCache._findOverlapping(expired, bboxPartial, 1, now).length,
    0,
...
{
  const entries = [
    { id: 1, bbox: bboxC, queryVersion: 1, fetchedAt: now, data: 'C' },
  ]; // disjoint
  ...
  assertEq(
    plan.fetchBBox,
    bboxPartial,
    '_planFetch — no overlap → fetchBBox is the request itself',
  );
}
...
{
  const entries = [
    { id: 5, bbox: bboxA, queryVersion: 1, fetchedAt: now, data: 'A' },
  ];
  const plan = OsmCache._planFetch(entries, bboxPartial, 1, now);
  const expectedUnion = OsmCache._unionBBox([bboxPartial, bboxA]);
  assertEq(
    plan.mergeIds.length,
    1,
    '_planFetch — one overlapping entry → one mergeId',
  );
  assertEq(
    plan.mergeIds[0],
    5,
    '_planFetch — mergeId matches the overlapping entry',
  );
  assertEq(
    plan.fetchBBox.minLat,
    expectedUnion.minLat,
    '_planFetch — fetchBBox union minLat correct',
  );
  assertEq(
    plan.fetchBBox.maxLat,
    expectedUnion.maxLat,
    '_planFetch — fetchBBox union maxLat correct',
  );
  assertEq(
    plan.fetchBBox.minLon,
    expectedUnion.minLon,
    '_planFetch — fetchBBox union minLon correct',
  );
  assertEq(
    plan.fetchBBox.maxLon,
    expectedUnion.maxLon,
    '_planFetch — fetchBBox union maxLon correct',
  );
}
...
// 3h. _planFetch — multiple overlapping entries all merged into one union fetch
{
  ...
  assertEq(
    plan.mergeIds.slice().sort().join(','),
    '1,9',
    '_planFetch — both overlapping entries superseded',
  );
  assertEq(
    plan.fetchBBox.minLat,
    expectedUnion.minLat,
    '_planFetch — multi-entry union minLat correct',
  );
  assertEq(
    plan.fetchBBox.maxLon,
    expectedUnion.maxLon,
    '_planFetch — multi-entry union maxLon correct',
  );
}
...
// 3i. _planFetch — oversized union skips the merge entirely
{
  ...
  assert(
    OsmCache._bboxIntersects(huge, bboxPartial),
  ...
  const entries = [
    { id: 1, bbox: huge, queryVersion: 1, fetchedAt: now, data: 'huge' },
  ];
  const plan = OsmCache._planFetch(entries, bboxPartial, 1, now);
  assertEq(
    plan.mergeIds.length,
    0,
    '_planFetch — oversized union → merge skipped, no mergeIds',
  );
  assertEq(
    plan.fetchBBox,
    bboxPartial,
    '_planFetch — oversized union → falls back to fetching just the request',
  );
}
...
// 4b. Over the cap → evicts the oldest-lastAccess entries first
{
  ...
  assertEq(
    evictions.length,
    2,
    '_selectEvictions — evicts exactly (count - cap) entries',
  );
  assertEq(
    evictedIds.join(','),
    'b,c',
    '_selectEvictions — evicts the two least-recently-accessed entries',
  );
}
...
// 4c. Exactly at the cap → nothing evicted
{
  ...
  assertEq(
    evictions.length,
    0,
    '_selectEvictions — exactly at cap → no evictions',
  );
}
