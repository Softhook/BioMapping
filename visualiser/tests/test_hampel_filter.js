...
test('applyHampelFilter: edge cases (empty array, null, window <= 1)', () => {
  ...
  assert.deepStrictEqual(
    GsrFilter.applyHampelFilter(arr, NaN),
    [1.0, 2.0, 3.0],
  );
});
...
test('applyHampelFilter: eliminates isolated spike outliers on flat baseline', () => {
  ...
  const raw = [
    5.0, 5.0, 5.0, 5.0, 5.0, 5.0, 5.0, 15.0, 5.0, 5.0, 5.0, -5.0, 5.0, 5.0, 5.0,
  ];
  ...
  assert.strictEqual(
    cleaned[7],
    5.0,
    'positive spike replaced with local median',
  );
  assert.strictEqual(
    cleaned[11],
    5.0,
    'negative spike replaced with local median',
  );
...
test('applyHampelFilter: preserves true SCR peak apex without blunting (unlike median)', () => {
  ...
  assert.ok(
    medianMax < 7.0,
    `Median filter blunted peak apex from 7.0 down to ${medianMax}`,
  );
  ...
  assert.strictEqual(
    hampelMax,
    7.0,
    'Hampel filter preserved true SCR peak apex at 7.0',
  );
  assert.deepStrictEqual(
    hampelFiltered,
    scr,
    'All points on smooth SCR curve remain untouched by Hampel',
  );
});
...
test('applyHampelFilter: removes spike sitting on an SCR peak while keeping peak', () => {
  ...
  const scrWithGlitch = [
    5.0, 5.2, 5.6, 6.2, 6.8, 14.0, 6.7, 6.3, 5.8, 5.4, 5.1,
  ];
  const cleaned = GsrFilter.applyHampelFilter(scrWithGlitch, 5);

  assert.ok(
    cleaned[5] < 10.0,
    `Glitch of 14.0 replaced with local median (${cleaned[5]})`,
  );
