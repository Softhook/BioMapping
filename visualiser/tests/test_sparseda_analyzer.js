...
test('GSRAnalyzer activates SparsEDA via useSparsEDA: true toggle directly', () => {
  ...
  assert.strictEqual(analyzer.phasicDriver.length, analyzer.raw.length);
  assert.strictEqual(analyzer.phasicClean.length, analyzer.raw.length);
  assert.ok(analyzer.phasicDriver.some(d => d.val > 0), 'useSparsEDA should produce sparse driver activity');
  assert.ok(analyzer.peaks.length >= 2, `expected at least 2 detected peaks, got ${analyzer.peaks.length}`);
});

test('SparsEDA annotates peaks with speed profiling and generates track dynamics stats', () => {
  const csvText = buildSyntheticCSV([
    { onsetSec: 15, amplitude: 0.25 },
    { onsetSec: 35, amplitude: 0.30 },
    { onsetSec: 55, amplitude: 0.20 },
  ...
  assert.ok(analyzer.peaks.length >= 4, `expected at least 4 peaks, got ${analyzer.peaks.length}`);
  ...
  for (const peak of analyzer.peaks) {
    ...
    assert.ok(typeof peak.scaleFactor === 'number', `peak at ${peak.time} should have numeric scaleFactor`);
    assert.ok(peak.bandIdx >= 0 && peak.bandIdx <= 4, `peak at ${peak.time} should have valid bandIdx`);
  }
});
...
test('SparsEDA runs without premature truncation on the fixture track', () => {
  ...
  assert.strictEqual(analyzer.phasicDeconvTruncated, false, 'Continuous track should not be flagged as truncated');
  assert.ok(analyzer.peaks.length > 50, `Expected > 50 peaks with fixed LARS on fixture track, got ${analyzer.peaks.length}`);
  assert.ok(analyzer.sparsedaStats, 'Track dynamics stats should be computed');
  assert.ok(analyzer.sparsedaStats.dominantSpeed, 'Dominant speed should be computed');

  const tonicAboveSignal = analyzer.tonic.filter((d, i) => d.val > analyzer.filtered[i].val + 1e-6).length;
  assert.strictEqual(tonicAboveSignal, 0, 'SparsEDA tonic baseline must never exceed the filtered signal (lower envelope)');
});
