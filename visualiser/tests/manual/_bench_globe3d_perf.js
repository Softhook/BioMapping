...
try {
  ...
  console.log(
    '  geometry pipeline. It is not committed and not part of `npm test`:\n',
  );
  console.log(
    '    npm i --no-save cesium@1.120 && node tests/manual/_bench_globe3d_perf.js\n',
  );
  process.exit(0);
}
...
const gpsParams = vm.runInContext(
...
console.log(
  `  drawPoints=${drawPoints.length}  segments=${segCount}  peaks=${(analyzer.peaks || []).length}  ` +
    `metric=${metric}  range=[${(+colorRange.min).toPrecision(3)}, ${(+colorRange.max).toPrecision(3)}]\n`,
);

if (drawPoints.length < 2) {
  console.log('  track has < 2 display points — nothing to profile\n');
  process.exit(0);
}
...
const heightSeries =
  heightMetric === metric ? colorSeries : seriesFor(heightMetric);
const baseHeight = 2.0,
  extrusionScale = 8.0;
let minV = colorRange.min,
  maxV = colorRange.max;
...
const heightAt = (idx) =>
  baseHeight + Math.max(0, heightSeries[idx] ?? 0) * extrusionScale;
...
function buildPerSegmentInstances(vf) {
  ...
  for (let i = 0; i < drawPoints.length - 1; i++) {
    ...
    const v1 = colorSeries[p1.origIdx] ?? minV,
      v2 = colorSeries[p2.origIdx] ?? minV;
    const hex = MapColors.getColorForMetric(metric, (v1 + v2) / 2, minV, maxV);
    const color = Cx.Color.fromCssColorString(hex).withAlpha(0.85);
    try {
      inst.push(
        new Cx.GeometryInstance({
          geometry: new Cx.WallGeometry({
            positions: [
              Cx.Cartesian3.fromDegrees(p1.lon, p1.lat),
              Cx.Cartesian3.fromDegrees(p2.lon, p2.lat),
            ],
            minimumHeights: [0, 0],
            maximumHeights: [heightAt(p1.origIdx), heightAt(p2.origIdx)],
            vertexFormat: vf,
          }),
          attributes: {
            color: Cx.ColorGeometryInstanceAttribute.fromColor(color),
          },
          id: `s-${i}`,
        }),
      );
...
function buildRealInstances(wallMaxSegments) {
  ...
  Object.assign(mgr, {
    activeColoringMetric: metric,
    heightMetric,
    externalColorRange: { min: minV, max: maxV },
    baseHeight,
    extrusionScale,
    wallMaxSegments,
    showGroundPath: false,
    requestRenderMode: false,
    _cesiumColorLut: null,
    _cesiumColorLutKey: null,
    viewer: {
      scene: {
        primitives: {
          add(p) {
...
function realizeAndCombine(instances) {
  ...
  if (realized.length)
    Cx.PrimitivePipeline.combineGeometry({ instances: realized });
  return realized;
}
function totals(realized) {
  ...
  for (const r of realized) {
    v += r.geometry.attributes.position.values.length / 3;
    idx += r.geometry.indices.length;
  }
...
const VARIANTS = [
  [
    'per-segment · pos+normal   (pre-refactor)',
    () => buildPerSegmentInstances(VF_NORMAL),
  ],
  [
    'per-segment · pos only     (flat format)',
    () => buildPerSegmentInstances(VF_FLAT),
  ],
...
console.log(
  '── SECTION A — real geometry realization (WallGeometry.createGeometry + combineGeometry) ──\n',
);
const resA = [];
for (const [label, build] of VARIANTS) {
  const t = totals(realizeAndCombine(build()));
  const r = bench(label, 2, 12, () => realizeAndCombine(build()));
  r.instances = build().length;
  r.verts = t.verts;
  r.tris = t.tris;
  resA.push(r);
  printRow(
    r,
    `instances=${String(r.instances).padStart(5)}  verts=${String(t.verts).padStart(7)}  tris=${String(t.tris).padStart(7)}`,
  );
}
{
  const base = resA[0],
    cur = resA[resA.length - 1];
  console.log(
    `\n  → current is ${(base.median / cur.median).toFixed(1)}x faster to realize+combine than pre-refactor`,
  );
  console.log(
    `    (flat format alone: ${(resA[0].median / resA[1].median).toFixed(1)}x · coalescing: ${(resA[1].median / resA[2].median).toFixed(1)}x · thinning: ${(resA[2].median / resA[3].median).toFixed(1)}x)`,
  );
  console.log(
    `    vertices ${base.verts} → ${cur.verts}  (${(base.verts / Math.max(1, cur.verts)).toFixed(1)}x fewer to upload / draw)\n`,
  );
}
...
function stubCesium() {
  ...
  C.Cartesian3.fromDegreesArray = (a) => {
    const o = [];
    for (let i = 0; i < a.length; i += 2) {
      counters.fromDegrees++;
      o.push({ lon: a[i], lat: a[i + 1] });
    }
    return o;
  };
  C.Cartesian3.fromDegreesArrayHeights = (a) => {
    const o = [];
    for (let i = 0; i < a.length; i += 3) {
      counters.fromDegrees++;
      o.push({ x: a[i], y: a[i + 1], z: a[i + 2] });
    }
    return o;
  };
  ...
  Color.fromCssColorString = (s) => {
    counters.colorParse++;
    return { _s: s, withAlpha: () => ({ _s: s }) };
  };
  Color.WHITE = { withAlpha: () => ({}) };
  C.Color = Color;
  C.ColorGeometryInstanceAttribute = { fromColor: (c) => c };
  C.WallGeometry = function (o) {
    counters.WallGeometry++;
    this._o = o;
  };
  C.EllipsoidGeometry = function (o) {
    counters.EllipsoidGeometry++;
    this._o = o;
  };
  C.PolygonGeometry = function (o) {
    this._o = o;
  };
  C.PolygonHierarchy = function (p) {
    this.positions = p;
  };
  C.GeometryInstance = function (o) {
    counters.GeometryInstance++;
    this._o = o;
    return this;
  };
  C.Primitive = function (o) {
    counters.Primitive++;
    this._o = o;
  };
  C.PerInstanceColorAppearance = function (o) {
    this._o = o;
  };
  C.PerInstanceColorAppearance.VERTEX_FORMAT = {};
  C.PerInstanceColorAppearance.FLAT_VERTEX_FORMAT = {};
  C.PolylineGlowMaterialProperty = function (o) {
    this._o = o;
  };
...
console.log('── SECTION B — main-thread JS per rebuild (stub Cesium) ──\n');
{
  ...
  printRow(
    r,
    counts +
      '  (colorParse 0 on redraw = LUT reused; ≤30 on a metric/range change)',
  );
  ...
  printRow(
    rH,
    `${(analyzer.memorableEvents || []).length} memorable events · ${hotCounts}`,
  );
  ...
  printRow(
    rAll,
    'wall geometry realize/upload is async (SECTION A) — not in this number',
  );
  ...
  printRow(
    r2,
    `series length ${(analyzer[SERIES_FIELD_GUESS(metric)] || analyzer.raw).length} — memoised; 1x/render`,
  );

  mgr.rfMode = 'triband';
  global.GSRGlobe3DRf = require(
    path.join(APP_DIR, 'src', 'map', 'globe3d', 'rf_expanse.js'),
  ).GSRGlobe3DRf;
  const r4 = bench('render3DRfExpanse', 3, 40, () => {
    mgr.clearRfEntities();
  ...
  mgr.currentDrawPoints = drawPoints;
  const r3 = bench('_pickTrackPoint x500', 3, 20, () => {
    for (let k = 0; k < 500; k++) mgr._pickTrackPoint({ x: k, y: k });
  });
  printRow(
    r3,
    `${drawPoints.length}-point scan/call, rAF-coalesced to ~1/frame`,
  );
  mgr.destroy();
}
