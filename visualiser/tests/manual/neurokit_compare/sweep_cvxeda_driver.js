...
const D = {
  ...
  useGaitFilter: process.env.BIOMAP_USE_GAIT_FILTER === '1',
};
...
if (trackPaths.length === 0) {
  console.error(
    'Usage: node sweep_cvxeda_driver.js <track1.csv> [track2.csv ...]',
  );
  process.exit(1);
}
...
for (const csvPath of trackPaths) {
  ...
  if (!fs.existsSync(gtPath)) {
    console.warn(`Skipping ${stem}: no ground-truth file`);
    continue;
  }
  ...
  const kernel = SCRDeconvolution.buildSCRFKernel(
    analyzer.sampleRate,
    tauSlow,
    tauFast,
    scf.kernelSec || 5.0,
...
console.log(
  `Decomposed ${tracks.length} track(s); sweeping driver-detection parameters...`,
);
    ...
    for (const cvxApexSearchHalfWinSec of apexHalfWinSecValues) {
      ...
      for (const track of tracks) {
        const candidateIndices = resolveCvxCandidates(
          track.driverVals,
          track.cleanVals,
          track.analyzer.sampleRate,
          track.kPeakIdx,
          cvxImpulseThreshold,
          cvxMinImpulseGapSec,
          cvxApexSearchHalfWinSec,
        );
...
function print(label, c) {
  ...
  console.log(
    `${label}: impulseThreshold=${c.cvxImpulseThreshold}, minGap=${c.cvxMinImpulseGapSec}s, apexWin=+/-${c.cvxApexSearchHalfWinSec}s`,
  );
  console.log(
    `  recall ${pct(c.recall)}  precision ${pct(c.precision)}  F1 ${c.f1.toFixed(3)}  TP ${c.tp} FN ${c.fn} FP ${c.fp}`,
  );
}
...
for (const c of candidates.slice(0, 10)) {
  console.log(
    `  impulseThreshold=${c.cvxImpulseThreshold}, minGap=${c.cvxMinImpulseGapSec}s, apexWin=+/-${c.cvxApexSearchHalfWinSec}s: F1 ${c.f1.toFixed(3)}, recall ${(c.recall * 100).toFixed(1)}%, precision ${(c.precision * 100).toFixed(1)}%, FP ${c.fp}`,
  );
}
...
for (const c of candidates.slice(0, 10)) {
  ...
  const flag =
    loss && loss.candidateTp < loss.productionTp
      ? `  <-- LOSES compound recall (${loss.productionTp}->${loss.candidateTp})`
      : '';
  console.log(
    `  impulseThreshold=${c.cvxImpulseThreshold}, minGap=${c.cvxMinImpulseGapSec}s, apexWin=+/-${c.cvxApexSearchHalfWinSec}s: compound TP ${loss ? loss.candidateTp : 'n/a'}${flag}`,
  );
}
