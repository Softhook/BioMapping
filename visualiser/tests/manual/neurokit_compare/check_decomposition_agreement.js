...
if (!jsonPath || !csvPath) {
  console.error(
    'Usage: node check_decomposition_agreement.js <python.json> <track.csv>',
  );
  process.exit(1);
}
...
if (py.cvx_tonic && py.cvx_phasic) {
  const alphaOverride = process.env.CVXEDA_ALPHA
    ? parseFloat(process.env.CVXEDA_ALPHA)
    : null;
  ...
  console.log(
    ` -- cvxEDA family (same published algorithm both sides, alpha=${alphaUsed}${alphaOverride != null ? ' [CVXEDA_ALPHA override]' : ' [production default]'}) --`,
  );
  ...
  console.log(
    ' -- cvxEDA family: no NeuroKit2 reference available (cvxopt missing?) --',
  );
}
