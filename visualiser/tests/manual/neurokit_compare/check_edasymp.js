...
const ours = [],
  theirs = [];
console.log(
  '=== EDASymp (0.045-0.25 Hz) agreement — ours (SpectralEDA) vs NeuroKit2 posada2016 ===',
);
for (const name of names) {
  ...
  ours.push(s.sympathetic);
  theirs.push(ref.eda_sympathetic);
  console.log(
    `  ${name}: NK=${ref.eda_sympathetic.toFixed(6)} ours=${s.sympathetic.toFixed(6)} ratio=${ratio.toFixed(4)}  (n=${ref.n_samples}, fs=${ref.sampling_rate.toFixed(2)})`,
  );
}
