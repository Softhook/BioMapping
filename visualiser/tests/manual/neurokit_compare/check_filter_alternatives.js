...
GsrFilter.decomposeTonicPhasic = (...args) => {
...
function whittakerSmooth(y, lambda) {
  ...
  const a0 = new Float64Array(n),
    a1 = new Float64Array(n - 1),
    a2 = new Float64Array(n - 2);
  ...
  const l0 = new Float64Array(n),
    l1 = new Float64Array(n - 1),
    l2 = new Float64Array(n - 2);
  for (let i = 0; i < n; i++) {
    let s = a0[i];
...
function boxzenSmooth(arr, sizeSec, fs, prefilterHz) {
  const pre = prefilterHz
    ? GsrFilter.applyZeroPhaseButterworth(arr, prefilterHz, 4, fs)
    : arr;
...
function amplitudeStats(pairs) {
  ...
  if (n === 0)
    return { n: 0, meanAbsErr: NaN, meanRelErr: NaN, r: NaN, slope: NaN };
  let sumAbs = 0,
    sumRelAbs = 0;
  let sumD = 0,
    sumT = 0,
...
const useWhittaker = (lambda) => () => {
  GsrFilter.applyZeroPhaseMovingAverage = (arr) => whittakerSmooth(arr, lambda);
};
...
const useButterworth = (cutoffHz, order) => () => {
  GsrFilter.applyZeroPhaseMovingAverage = (arr) =>
    GsrFilter.applyZeroPhaseButterworth(
      arr,
      cutoffHz,
      order,
      SYNTH_SAMPLE_RATE,
...
const useBoxzen = (sizeSec, prefilterHz) => () => {
  GsrFilter.applyZeroPhaseMovingAverage = (arr) =>
    boxzenSmooth(arr, sizeSec, SYNTH_SAMPLE_RATE, prefilterHz);
};
...
const useButterworthPlusBox = (cutoffHz, order, boxSec) => () => {
  GsrFilter.applyZeroPhaseMovingAverage = (arr) => {
    const afterButter = GsrFilter.applyZeroPhaseButterworth(
      arr,
      cutoffHz,
      order,
      SYNTH_SAMPLE_RATE,
...
const useLR4 = (cutoffHz) => () => {
  GsrFilter.applyZeroPhaseMovingAverage = (arr) =>
    GsrFilter.applyZeroPhaseLinkwitzRiley(arr, cutoffHz, SYNTH_SAMPLE_RATE);
};
...
const CANDIDATES = [
  ...
  [
    'none (lpfWindow=0)',
    useOriginalBox,
    { lpfWindow: 0, usePeakProminence: true },
  ],
  [
    'box 0.5s (box filter default)',
    useOriginalBox,
    { lpfWindow: 0.5, usePeakProminence: true },
  ],
  ['box 0.7s', useOriginalBox, { lpfWindow: 0.7, usePeakProminence: true }],
  ['box 0.9s', useOriginalBox, { lpfWindow: 0.9, usePeakProminence: true }],
  [
    'box 1.1s (manual gait fix)',
    useOriginalBox,
    { lpfWindow: 1.1, usePeakProminence: true },
  ],
  ...
  [
    'whittaker lambda=5',
    useWhittaker(5),
    { lpfWindow: 1, usePeakProminence: true },
  ],
  [
    'whittaker lambda=25',
    useWhittaker(25),
  ...
  [
    'whittaker lambda=150',
    useWhittaker(150),
    { lpfWindow: 1, usePeakProminence: true },
  ],
  [
    'whittaker lambda=200',
    useWhittaker(200),
    { lpfWindow: 1, usePeakProminence: true },
  ],
  [
    'whittaker lambda=350',
    useWhittaker(350),
    { lpfWindow: 1, usePeakProminence: true },
  ],
  [
    'whittaker lambda=500',
    useWhittaker(500),
    { lpfWindow: 1, usePeakProminence: true },
  ],
  [
  ...
  // ground truth too.
  [
    'butterworth 2.0Hz order4',
    useButterworth(2.0, 4),
    { lpfWindow: 1, usePeakProminence: true },
  ],
  [
    'butterworth 1.5Hz order4',
    useButterworth(1.5, 4),
    { lpfWindow: 1, usePeakProminence: true },
  ],
  [
    'butterworth 1.2Hz order4',
    useButterworth(1.2, 4),
    { lpfWindow: 1, usePeakProminence: true },
  ],
  [
    'butterworth 1.0Hz order4',
    useButterworth(1.0, 4),
    { lpfWindow: 1, usePeakProminence: true },
  ],
  [
    'butterworth 0.8Hz order4',
    useButterworth(0.8, 4),
    { lpfWindow: 1, usePeakProminence: true },
  ],
  [
    'butterworth 1.2Hz order2',
    useButterworth(1.2, 2),
    { lpfWindow: 1, usePeakProminence: true },
  ],
  [
    'butterworth 1.0Hz order2',
    useButterworth(1.0, 2),
    { lpfWindow: 1, usePeakProminence: true },
  ],
  [
    'butterworth 0.8Hz order2',
    useButterworth(0.8, 2),
    { lpfWindow: 1, usePeakProminence: true },
  ],
  ...
  [
    'butter 1.0Hz o4 + box 0.1s',
    useButterworthPlusBox(1.0, 4, 0.1),
    { lpfWindow: 1, usePeakProminence: true },
  ],
  [
    'butter 1.0Hz o4 + box 0.2s',
    useButterworthPlusBox(1.0, 4, 0.2),
    { lpfWindow: 1, usePeakProminence: true },
  ],
  [
    'butter 1.0Hz o4 + box 0.3s',
    useButterworthPlusBox(1.0, 4, 0.3),
    { lpfWindow: 1, usePeakProminence: true },
  ],
  [
    'butter 0.8Hz o4 + box 0.2s',
    useButterworthPlusBox(0.8, 4, 0.2),
    { lpfWindow: 1, usePeakProminence: true },
  ],
  [
    'butter 0.8Hz o4 + box 0.3s',
    useButterworthPlusBox(0.8, 4, 0.3),
    { lpfWindow: 1, usePeakProminence: true },
  ],
  [
    'butter 1.0Hz o2 + box 0.1s',
    useButterworthPlusBox(1.0, 2, 0.1),
    { lpfWindow: 1, usePeakProminence: true },
  ],
  [
    'butter 1.0Hz o2 + box 0.2s',
    useButterworthPlusBox(1.0, 2, 0.2),
    { lpfWindow: 1, usePeakProminence: true },
  ],
  [
    'butter 0.8Hz o2 + box 0.1s',
    useButterworthPlusBox(0.8, 2, 0.1),
    { lpfWindow: 1, usePeakProminence: true },
  ],
  [
    'LR4 0.9Hz (cascaded o2)',
    useLR4(0.9),
    { lpfWindow: 1, usePeakProminence: true },
  ],
  [
    'LR4 1.0Hz (cascaded o2)',
    useLR4(1.0),
    { lpfWindow: 1, usePeakProminence: true },
  ],
  [
    'LR4 1.1Hz (cascaded o2)',
    useLR4(1.1),
    { lpfWindow: 1, usePeakProminence: true },
  ],
  [
    ...
    () => {
      GsrFilter.applyZeroPhaseMovingAverage = (arr) =>
        originalBoxLPF(GsrFilter.applyZeroPhaseLinkwitzRiley(arr, 1.0, 10), 2);
    },
    { lpfWindow: 1, usePeakProminence: true },
  ],
  ...
  [
    'MP deconv, no LPF',
    useOriginalBox,
    { lpfWindow: 0, usePeakProminence: false, useDeconvolution: true },
  ],
  [
    'MP deconv + box 0.5s',
    useOriginalBox,
    { lpfWindow: 0.5, usePeakProminence: false, useDeconvolution: true },
  ],
  [
    'cvxEDA, no LPF',
    useOriginalBox,
    { lpfWindow: 0, usePeakProminence: false, useCvxEDA: true },
  ],
  [
    'cvxEDA + box 0.5s',
    useOriginalBox,
    { lpfWindow: 0.5, usePeakProminence: false, useCvxEDA: true },
  ],
  // ── SHIPPED gait filter: GSR_DEFAULT (Linkwitz-Riley LR4 1.0Hz)
  [
    'SHIPPED useGaitFilter:true',
    useOriginalBox,
    { useGaitFilter: true, usePeakProminence: true },
  ],
];
...
if (!gtDir) {
  console.error(
    'Usage: node check_filter_alternatives.js <ground_truth_dir> [neurokit.json]',
  );
  process.exit(1);
}
...
const nkAll =
  nkPath && fs.existsSync(nkPath)
    ? JSON.parse(fs.readFileSync(nkPath, 'utf8'))
    : {};
...
for (const file of files) {
  ...
  const gt = JSON.parse(
    fs.readFileSync(path.join(gtDir, `${name}.ground_truth.json`), 'utf8'),
  );
  const trueScrs = gt.scrs;

  console.log(
    `=== ${name}: ${trueScrs.length} true SCRs (duration ${gt.params.duration}s, noise ${gt.params.noise}${gt.params.gait_freq ? `, gait ${gt.params.gait_freq}Hz@${gt.params.gait_amplitude}uS` : ''}) ===`,
  );
  ...
  if (nk)
    console.log(
      fmt(
        'NeuroKit2 (ref, 3Hz Butterworth)',
        score(nk.peak_times, nk.peak_amplitudes || [], trueScrs),
      ),
    );
  console.log();
}
