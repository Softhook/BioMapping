...
if (mode === 'dump') {
  ...
  console.error(
    `Dumped ${vals.length} phasic samples, ${results.length} recovery queries -> ${outPath}`,
  );
  ...
  for (let i = 0; i < nk.peak_index.length; i++)
    nkByPeak.set(nk.peak_index[i], nk.recovery_index[i]);

  const name = path.basename(process.argv[5] || '', '.csv');
  console.log(
    `=== ${name}: recovery-time agreement (same phasic/onset/amplitude, ours=first-crossing vs NeuroKit2=closest-value) ===`,
  );
  ...
  console.log(
    `  both found a recovery point: ${bothFound}  (exact index match: ${exact}, within 1.0s: ${within1s})`,
  );
  console.log(
    `  mean|time diff| where both found but differ: ${diffCount ? (sumAbsTimeDiff / diffCount).toFixed(4) : 0}s  max: ${maxAbsTimeDiff.toFixed(4)}s`,
  );
  ...
  console.error(
    'Usage: node check_recovery_agreement.js dump <track.csv> <out.json>',
  );
  console.error(
    '       node check_recovery_agreement.js compare <out.json> <nk_result.json> [track.csv]',
  );
