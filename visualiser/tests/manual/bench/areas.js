...
const B = (opts, over = {}) => ({
  ...
  iters: 10,
  ...over,
  ...(opts.iters ? { iters: opts.iters } : {}),
  ...(opts.warmup ? { warmup: opts.warmup } : {}),
});
...
const analyze = {
  name: 'analyze',
  title:
    'analyze() — recompute per settled GSR-slider frame (HIT = peak slider · MISS = filter slider)',
  perTrack: true,
  columns: [
    { key: 'rows', label: 'rows' },
  ...
  run({ h, window, track, GSR_CONST, opts }) {
    ...
    const miss = h.bench(
      () =>
        analyzer.analyze(
          { ...params, lpfWindow: params.lpfWindow + (k++ % 5) * 1e-3 },
          0,
        ),
      B(opts, { iters: 8 }),
    );
...
const signalMetrics = {
  name: 'signal-metrics',
  title:
    'continuous metrics — peakDensity + phasicAUC + arousalIndex + triIndex (graphView change / slider)',
  perTrack: true,
  columns: [
    { key: 'rows', label: 'rows' },
    { key: 'totalMs', label: 'all 4 ms' },
  ],
  run({ h, track, opts }) {
...
const arousalPlaces = {
  name: 'arousal-places',
  title:
    'Arousal Places recompute — compactClusters + buildPlaces + every getConcaveBlob (peak/merge slider, exclusion)',
  perTrack: true,
  columns: [
    { key: 'peaks', label: 'peaks' },
    { key: 'clusters', label: 'clstr' },
    { key: 'places', label: 'places' },
    { key: 'clusterMs', label: 'cluster' },
    { key: 'buildMs', label: 'build' },
    { key: 'blobMs', label: 'blobs' },
  ...
  run({ h, window, map, mapManager, track, GSR_CONST, opts }) {
    ...
    const scoreTracks = [
      { id: track.id, sampleRate: a.sampleRate, raw: a.raw, phasic: a.phasic },
    ];
    const meanAmp =
      pts.reduce((s, p) => s + (p.amplitude || 0), 0) / Math.max(1, pts.length);

    let clusters;
    const bc = h.bench(
      () => {
        clusters = SC.compactClusters(pts, 35, 1.8);
      },
      B(opts, { iters: 12 }),
    );
    const bb = h.bench(
      () => AP.buildPlaces(clusters, scoreTracks, GSR_CONST.AROUSAL_PLACES),
      B(opts, { iters: 8 }),
    );
    const places = AP.buildPlaces(
      clusters,
      scoreTracks,
    ...
    const bl = h.bench(
      () => {
        for (const pl of places)
          SC.getConcaveBlob(pl.cluster, 12.25, 17.5, meanAmp);
      },
      B(opts, { iters: 6 }),
    );
...
const labelPlacement = {
  ...
  title:
    'GSRLabelManager.computeLabelPositions — 360° label collision (runs every peak render, scales with labelled count)',
  perTrack: true,
  columns: [
    { key: 'peaks', label: 'peaks' },
    { key: 'n25Ms', label: '25 lbl ms' },
    { key: 'n100Ms', label: '100 lbl ms' },
    { key: 'allMs', label: 'all lbl ms' },
  ],
  run({ h, window, map, mapManager, track, GSR_CONST, opts }) {
    ...
    const c25 = cand(25),
      ...
      cAll = cand(px.length);
    const time = (arr) =>
      h.bench(() => LM.computeLabelPositions(arr), B(opts, { iters: 10 }))
        .median;
    return {
      peaks: px.length,
      n25Ms: time(c25),
...
const gpsPipeline = {
  name: 'gps-pipeline',
  title:
    '_getOrBuildDrawPoints — a GPS-slider frame: collect + HDOP/speed gates + Kalman RTS + reconstruct + downsample + RDP',
  perTrack: true,
  columns: [
    { key: 'rows', label: 'rows' },
...
const renderSingle = {
  name: 'render-single',
  title:
    'single-track map — renderData() full · refreshPeakMarkers() warm cache · refreshPeakMarkers() forced miss',
  perTrack: true,
  columns: [
    { key: 'peaks', label: 'peaks' },
    { key: 'renderMs', label: 'renderData' },
  ...
  run({ h, window, mapManager, track, GSR_CONST, opts }) {
    ...
    const render = h.bench(
      () => mapManager.renderData(track.analyzer, p),
      B(opts, { iters: 8 }),
    );
    const warm = h.bench(
      () =>
        mapManager.refreshPeakMarkers(track.analyzer, p, {
...
const graphDraw = {
  name: 'graph-draw',
  title:
    'p5 draw() — the full graph frame that runs on every hover / scrub / redraw (~60fps budget = 16.6ms)',
  perTrack: true,
  columns: [
    { key: 'rows', label: 'rows' },
    { key: 'fullMs', label: 'full-zoom ms' },
    { key: 'winMs', label: '60s-window ms' },
    { key: 'pctBudget', label: '% of 16.6ms' },
  ],
  run({ h, window, track, opts }) {
    ...
    A.showRaw =
      ...
      A.showHotspots =
        true;
    A.totalDuration =
      track.analyzer.raw.length / (track.analyzer.sampleRate || 10);

    A.viewStartTime = 0;
    A.viewDuration = A.totalDuration;
    const full = h.bench(() => window.draw(), B(opts, { iters: 20 }));
    A.viewStartTime = Math.max(0, A.totalDuration / 2 - 30);
    A.viewDuration = 60;
...
const contourSurface = {
  ...
  title:
    'generateContourSurface() — collective topography grid (contour slider); IDW splat vs "peaks" KDE',
  perTrack: false,
  columns: [
    { key: 'tracks', label: 'tracks' },
    { key: 'totalPeaks', label: 'peaks' },
    { key: 'idwMs', label: 'IDW ms' },
    { key: 'peaksMs', label: 'peaks-mode ms' },
  ],
  run({ h, window, mapManager, tracks, GSR_CONST, opts }) {
    ...
    const idw = h.bench(
      () => cm.generateContourSurface({ ...base, topographySource: 'phasic' }),
      B(opts, { iters: 6 }),
    );
    const pk = h.bench(
      () => cm.generateContourSurface({ ...base, topographySource: 'peaks' }),
      B(opts, { iters: 6 }),
    );
    const totalPeaks = tracks.reduce((s, t) => s + t.peaks, 0);
    return [
      {
        track: `${tracks.length}-track set`,
...
const renderCollective = {
  ...
  title:
    'renderCollectiveData() COLD — every "enter collective / add track / exclusion / merge slider" pays this',
  perTrack: false,
  columns: [
    { key: 'tracks', label: 'tracks' },
    { key: 'totalPeaks', label: 'peaks' },
