...
console.log(
  `=== gait-band (1.4-2.0Hz) speed-correlation: raw vs box 0.5s vs gait filter, across ${files.length} real tracks ===\n`,
);
console.log(
  'track'.padEnd(18) +
    'nWin'.padStart(6) +
    ...
    'r(box)'.padStart(9) +
    'r(gait)'.padStart(9) +
    'brisk%(raw)'.padStart(13) +
    'brisk%(box)'.padStart(13) +
    'brisk%(gait)'.padStart(14),
);
...
for (const file of files) {
  ...
  try {
    ...
    noneRes = slidingSpeedCorrelation(
      noneP.phasicVals,
      speeds,
      noneP.sampleRate,
    );
    gaitRes = slidingSpeedCorrelation(
      gaitP.phasicVals,
      speeds,
      gaitP.sampleRate,
    );
  ...
  if (noneRes.n < 15 || isNaN(noneRes.r) || isNaN(gaitRes.r) || isNaN(boxRes.r))
    continue;
  rows.push({ file, noneRes, gaitRes, boxRes });
  console.log(
    file.padEnd(18) +
      String(noneRes.n).padStart(6) +
      noneRes.r.toFixed(3).padStart(9) +
      boxRes.r.toFixed(3).padStart(9) +
      gaitRes.r.toFixed(3).padStart(9) +
...
const meanAbsR = (side) =>
  rows.reduce((s, r) => s + Math.abs(r[side].r), 0) / rows.length;
const briskRows = rows.filter(
  (r) =>
    !isNaN(r.noneRes.briskMean) &&
    !isNaN(r.boxRes.briskMean) &&
    !isNaN(r.gaitRes.briskMean),
);
const meanBrisk = (side) =>
  briskRows.length
    ? (briskRows.reduce((s, r) => s + r[side].briskMean, 0) /
        briskRows.length) *
      100
    : 0;
...
console.log(
  `mean |r| (speed vs gait-band power):     raw=${meanAbsR('noneRes').toFixed(3)}   box=${meanAbsR('boxRes').toFixed(3)}   gait=${meanAbsR('gaitRes').toFixed(3)}`,
);
console.log(
  `mean brisk-pace gait-band share:          raw=${meanBrisk('noneRes').toFixed(1)}%   box=${meanBrisk('boxRes').toFixed(1)}%   gait=${meanBrisk('gaitRes').toFixed(1)}%`,
);
console.log(
  `tracks where box reduces brisk-pace band share vs raw:  ${briskReducedVsNone('boxRes')}/${rows.length}`,
);
console.log(
  `tracks where gait filter reduces brisk-pace band share vs raw: ${briskReducedVsNone('gaitRes')}/${rows.length}`,
);
console.log(
  `tracks where gait filter's brisk-pace band share is lower than box's: ${rows.filter((r) => r.gaitRes.briskMean < r.boxRes.briskMean).length}/${rows.length}`,
);
