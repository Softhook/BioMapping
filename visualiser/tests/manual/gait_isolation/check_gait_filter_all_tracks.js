...
console.log(
  `=== ${files.length} real tracks: useGaitFilter:true vs box 0.5s ===\n`,
);
console.log(
  'track'.padEnd(18) +
    'meanSpeed'.padStart(10) +
    'peaks(gait)'.padStart(13) +
    'peaks(box)'.padStart(12) +
    'ratio'.padStart(8) +
    '  q(gait)'.padStart(10) +
    '  q(box)'.padStart(10) +
    'amp(gait)'.padStart(11) +
    'amp(box)'.padStart(10),
);
...
for (const file of files) {
  ...
  const ratio =
    box.peaks > 0 ? gait.peaks / box.peaks : gait.peaks > 0 ? Infinity : 1;
  rows.push({ file, meanSpeed, gait, box, ratio });
  console.log(
    file.padEnd(18) +
      (meanSpeed == null
        ? 'n/a'.padStart(10)
        : meanSpeed.toFixed(2).padStart(10)) +
...
const meanRatio =
  rows.reduce((s, r) => s + (isFinite(r.ratio) ? r.ratio : 0), 0) / rows.length;
const medianRatio = rows
  .map((r) => r.ratio)
  .filter(isFinite)
  .sort((a, b) => a - b)[Math.floor(rows.length / 2)];
const meanQGait =
  rows.reduce((s, r) => s + r.gait.meanQuality, 0) / rows.length;
const meanQBox = rows.reduce((s, r) => s + r.box.meanQuality, 0) / rows.length;

console.log('\n=== Summary ===');
console.log(
  `total peaks:   gait=${totalGait}   box=${totalBox}   (${((totalGait / totalBox - 1) * 100).toFixed(1)}%)`,
);
console.log(
  `mean ratio (gait/box) per track: ${meanRatio.toFixed(2)}   median: ${medianRatio.toFixed(2)}`,
);
console.log(
  `mean quality:  gait=${meanQGait.toFixed(3)}   box=${meanQBox.toFixed(3)}`,
);
...
const withSpeed = rows
  .filter((r) => r.meanSpeed != null)
...
if (withSpeed.length >= 8) {
  ...
  const avgRatio = (arr) =>
    arr.reduce((s, r) => s + (isFinite(r.ratio) ? r.ratio : 0), 0) / arr.length;
  const avgSpeed = (arr) =>
    arr.reduce((s, r) => s + r.meanSpeed, 0) / arr.length;
  console.log(
    `\nslowest quartile (n=${slow.length}, mean speed ${avgSpeed(slow).toFixed(2)} m/s): mean ratio ${avgRatio(slow).toFixed(2)}`,
  );
  console.log(
    `fastest quartile (n=${fast.length}, mean speed ${avgSpeed(fast).toFixed(2)} m/s): mean ratio ${avgRatio(fast).toFixed(2)}`,
  );
}
...
for (const r of worst) {
  console.log(
    `  ${r.file.padEnd(18)} ratio=${r.ratio.toFixed(2)}  meanSpeed=${r.meanSpeed == null ? 'n/a' : r.meanSpeed.toFixed(2)}  gaitPeaks=${r.gait.peaks}  boxPeaks=${r.box.peaks}`,
  );
}
...
const weightedQ = (side) => {
  ...
  for (const r of nonEmpty) {
    num += r[side].meanQuality * r[side].peaks;
    den += r[side].peaks;
  }
...
const withDelta = nonEmpty.map((r) => ({
  ...r,
  qDelta: r.gait.meanQuality - r.box.meanQuality,
}));
const smallN = withDelta.filter(
  (r) => Math.min(r.gait.peaks, r.box.peaks) < 15,
);
const bigN = withDelta.filter((r) => Math.min(r.gait.peaks, r.box.peaks) >= 15);
const avgDelta = (arr) =>
  arr.length ? arr.reduce((s, r) => s + r.qDelta, 0) / arr.length : NaN;
console.log(
  `\nquality delta (gait-box), tracks with <15 peaks (n=${smallN.length}): mean ${avgDelta(smallN).toFixed(4)}`,
);
console.log(
  `quality delta (gait-box), tracks with >=15 peaks (n=${bigN.length}): mean ${avgDelta(bigN).toFixed(4)}`,
);
...
for (const r of worstQ) {
  console.log(
    `  ${r.file.padEnd(18)} qDelta=${r.qDelta.toFixed(4)}  gaitPeaks=${r.gait.peaks}  boxPeaks=${r.box.peaks}  meanSpeed=${r.meanSpeed == null ? 'n/a' : r.meanSpeed.toFixed(2)}`,
  );
}
