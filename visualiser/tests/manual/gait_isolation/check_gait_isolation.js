...
function buildD2TD2(n) {
  const diag0 = new Float64Array(n),
    diag1 = new Float64Array(Math.max(0, n - 1)),
    diag2 = new Float64Array(Math.max(0, n - 2));
  for (let k = 0; k <= n - 3; k++) {
...
function slidingSpeedCorrelation(phasicVals, speeds, sampleRate) {
  ...
  for (
    let start = 0;
    start + windowSamples <= phasicVals.length;
    start += stepSamples
  ) {
    const speedSeg = speeds
    ...
    const share = bandShareOfSegment(
      phasicVals.slice(start, start + windowSamples),
      sampleRate,
      GAIT_FREQS,
      TOTAL_FREQS,
    );
  ...
  const medium = shareSamples.filter(
    (_, i) => speedSamples[i] >= 0.5 && speedSamples[i] < 1.0,
  );
  const brisk = shareSamples.filter(
    (_, i) => speedSamples[i] >= 1.0 && speedSamples[i] < 1.5,
  );
  const mediumMean = medium.length
    ? medium.reduce((a, b) => a + b, 0) / medium.length
    : NaN;
  const briskMean = brisk.length
    ? brisk.reduce((a, b) => a + b, 0) / brisk.length
    : NaN;
  return {
    r,
    mediumMean,
    briskMean,
    n: speedSamples.length,
    nMedium: medium.length,
    nBrisk: brisk.length,
  };
}
...
const useBox = (windowSec) => () => {
  GsrFilter.applyZeroPhaseMovingAverage = (arr) =>
    originalBoxLPF(arr, Math.round(windowSec * TRACK_SAMPLE_RATE));
};
...
const useButterworth = (cutoffHz, order) => () => {
  GsrFilter.applyZeroPhaseMovingAverage = (arr) =>
    originalButterworth(arr, cutoffHz, order, TRACK_SAMPLE_RATE);
};
const useBoxzen = (sizeSec, prefilterHz) => () => {
  GsrFilter.applyZeroPhaseMovingAverage = (arr) =>
    boxzenSmooth(arr, sizeSec, TRACK_SAMPLE_RATE, prefilterHz);
};
...
const useLR4 = (cutoffHz) => () => {
  GsrFilter.applyZeroPhaseMovingAverage = (arr) =>
    GsrFilter.applyZeroPhaseLinkwitzRiley(arr, cutoffHz, TRACK_SAMPLE_RATE);
};
...
const CANDIDATES = [
  ...
  [
    ...
    useBox(0.5),
    { lpfWindow: 1, useGaitFilter: false },
  ],
  ...
  [
    ...
    useProductionDefault,
    { lpfWindow: 1, useGaitFilter: true },
  ],
];

const targetTracks = process.argv.slice(2).length
  ? process.argv.slice(2)
  : ['biomap_024.csv', 'biomap_059.csv'];
...
for (const trackFile of targetTracks) {
  ...
  for (const [label, installFilter, patch] of CANDIDATES) {
    ...
    const { r, mediumMean, briskMean, nMedium, nBrisk } =
      slidingSpeedCorrelation(phasicVals, speeds, a.sampleRate);
    const meanQuality = a.peaks.length
      ? a.peaks.reduce((s, p) => s + p.qualityScore, 0) / a.peaks.length
      : 0;
    const sumAmp = a.peaks.reduce((s, p) => s + p.amplitude, 0);
    console.log(
      `  ${label.padEnd(28)} r=${(isNaN(r) ? '   NaN' : r.toFixed(3)).padStart(6)}  medium%=${(100 * (mediumMean || 0)).toFixed(1).padStart(5)}(n=${nMedium})  brisk%=${(100 * (briskMean || 0)).toFixed(1).padStart(5)}(n=${nBrisk})  gap=${(100 * ((briskMean || 0) - (mediumMean || 0))).toFixed(1).padStart(5)}pp   |   peaks=${String(a.peaks.length).padStart(4)}  meanQuality=${meanQuality.toFixed(3)}  sumAmp=${sumAmp.toFixed(1)}`,
    );
  }
}
GsrFilter.applyZeroPhaseMovingAverage = originalBoxLPF;
