...
const trackFiles = fs
  ...
  .map((name) => ({
    ...
    _text: fs.readFileSync(
      path.join(__dirname, '..', '..', '..', 'tracks', name),
      'utf8',
    ),
  }));
...
const check = () => {
  ...
  if (n >= trackFiles.length || stalled) {
    ...
    const loadedNames = new Set(
      window.AppState.collectiveManager.tracks.map((t) => t.name),
    );
    const missing = trackFiles
      .map((f) => f.name)
      .filter((fn) => !loadedNames.has(fn));
    ...
    for (let i = 0; i < n; i++) {
      ...
      console.log(
        `track ${String(i + 1).padStart(2)} ${track.name.padEnd(16)} peaks=${String(nPeaks).padStart(4)} onMap=${String(peakMarkers).padStart(4)} paths=${String(paths).padStart(4)} hotspot=${String(hotspots).padStart(4)} fixes=${String(nFixes).padStart(5)} rf=${isRf ? 'y' : 'n'}${flag}${errTag}`,
      );
    }

    const missingPeaks = peaks.filter(
      (p) => p.nPeaks > 0 && p.peakMarkers === 0,
    );
    console.log(
  ...
  if (Date.now() - start > 180000) {
    console.log(
      `timed out with ${n}/${trackFiles.length} tracks; errors=${errors.length}`,
    );
    return;
  }
