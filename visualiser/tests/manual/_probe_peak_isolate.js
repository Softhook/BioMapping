...
vm.runInContext(
  'RFFluidRenderer = undefined; GSRSpatialClustering = undefined;',
  context,
);
...
['BOLD', 'NORMAL', 'ITALIC', 'LIGHT'].forEach((c) => {
  window[c] = window[c] || c;
});
[
  'millis',
...
function installRecordingLeaflet() {
  const map = {
    ...
    _nextId: 1,
    addLayer(l) {
      if (!l || typeof l !== 'object') return map;
      if (l._gsrId === undefined) l._gsrId = map._nextId++;
      map._layers.set(l._gsrId, l);
      if (l._isGroup) {
    ...
    removeLayer(l) {
      if (!l || l._gsrId === undefined) return map;
      if (l._isGroup) {
        ...
        map._layers.delete(l._gsrId);
        map._viaGroup.delete(l);
        for (const g of map._groups.values())
          if (g.hasLayer(l)) g._children.delete(l._gsrId);
      }
      return map;
    },
    hasLayer(l) {
      if (!l || l._gsrId === undefined) return false;
      if (l._isGroup) return map._groups.has(l._gsrId);
      return map._direct.includes(l) || map._viaGroup.has(l);
    },
    latLngToLayerPoint() {
      return { x: 10, y: 20 };
    },
  ...
  const L = {
    ...
    DomUtil: {
      create: (t, c) => {
        const el = window.document.createElement(t);
        if (c) el.className = c;
        return el;
      },
      setTransform() {},
...
const trackFiles = [
  {
    name: TARGET,
    _text: fs.readFileSync(
      path.join(__dirname, '..', '..', '..', 'tracks', TARGET),
      'utf8',
    ),
  },
];
...
const check = () => {
  ...
  if (n >= trackFiles.length) {
    ...
    console.log(
      `Loaded ${track.name}; peaks=${anal.peaks.length}; raw=${anal.raw.length}`,
    );
    let nFixes = 0;
    for (const d of anal.raw)
      if (d._isGpsFix && !isNaN(d.lat) && !isNaN(d.lon)) nFixes++;
    console.log(`gps fixes (raw): ${nFixes}`);
    console.log(
      `filteredGps defined: ${!!anal.filteredGps}; length=${anal.filteredGps ? anal.filteredGps.length : 'n/a'}`,
    );
    if (anal.filteredGps) {
      let fFixes = 0;
    ...
    const li = window.document.querySelector(
      `li[data-track-id="${track.id}"] .track-details`,
    );
    ...
    try {
      ...
      console.log(
        'map counts after direct renderData:',
        JSON.stringify(countMap()),
      );
      const rl = mm.getRenderLayers();
      console.log(
        'getRenderLayers: paths=%d peakMarkers=%d hotspots=%d',
        rl.paths.length,
        rl.peakMarkers.length,
        rl.hotspots.length,
      );
    } catch (e) {
    ...
    try {
      const res = mm._getOrBuildDrawPoints(
        'iso',
        anal,
        window.GSRStorage.buildGpsParams(),
      );
      console.log(
        '_getOrBuildDrawPoints: gpsPoints=%d drawPoints=%d',
        res.gpsPoints.length,
        res.drawPoints.length,
      );
    } catch (e) {
      ...
      anal.peaks.forEach((peak, index) => {
        ...
        if (coords && !isNaN(coords.lat) && !isNaN(coords.lon)) {
          ...
          if (firstFew.length < 3)
            firstFew.push({ index, lat: coords.lat, lon: coords.lon });
        } else {
          unresolvable++;
        }
      });
      console.log(
        `peaks resolvable=${resolvable} unresolvable=${unresolvable}`,
      );
    ...
    try {
      ...
      console.log(
        `2 applyHdopGate(maxHdop=${p.maxHdop || 2.0}): ${pts.length} (dropped ${pts0.length - pts.length})`,
      );
      // breakdown of dropped by hdop value
      const droppedHdop = pts0.filter(
        (d) => !(isNaN(d.hdop) || d.hdop <= (p.maxHdop || 2.0)),
      );
      const hdopVals = {};
      droppedHdop.forEach((d) => {
        const k = d.hdop == null ? 'null' : d.hdop;
        hdopVals[k] = (hdopVals[k] || 0) + 1;
      });
  ...
  if (Date.now() - start > 60000) {
    console.log('timed out waiting for load');
    return;
  }
  setTimeout(check, 25);
};
