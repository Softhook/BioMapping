...
function state(label) {
  ...
  console.log(
    `${label.padEnd(46)} btn.active=${btn.classList.contains('active')} disabled=${btn.hasAttribute('disabled')} showRFFluid=${mm.showRFFluid} renderer.visible=${r ? r.options.visible : 'no-renderer'}`,
  );
}
...
if (!buttonPressedB && !fluidRendersB) {
  console.log(
    '  OK: user toggled off — fluid stays hidden, button stays unpressed',
  );
} else {
