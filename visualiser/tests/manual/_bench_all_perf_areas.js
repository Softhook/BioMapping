...
vm.runInContext(
  'var width = 1200, height = 600, mouseX = 500, mouseY = 300, winMouseX = 500, winMouseY = 300, BOLD = "bold", NORMAL = "normal", ITALIC = "italic", circle = () => {}, ellipse = () => {}, triangle = () => {};',
  context,
);
window.width = 1200;
window.height = 600;
...
console.log(
  'Primary subject: Track 113 (biomap_113.csv, mid-sized) — worst case [2b]: biomap_016.csv',
);
console.log('='.repeat(78));
...
console.log(
  `  Sample Rate:    ${track113.analyzer.sampleRate} Hz (${(rawPoints113.length / track113.analyzer.sampleRate / 60).toFixed(1)} minutes duration)`,
);
console.log(
  `  Detected Peaks: ${track113.analyzer.peaks.length} total (${validPeaks113.length} valid active)`,
);
...
console.log(
  '    Full refresh with a WARM Arousal Places cache (repeat call, no input change)',
);
console.log(
  '    vs a forced cache MISS (peak amplitude nudged) vs skipClustering (label-edit path).',
);
...
const bRefreshWarm = bench(2, 6, () => {
  window.AppState.mapManager.refreshPeakMarkers(
    track113.analyzer,
    {},
...
console.log(
  `    refreshPeakMarkers, cache WARM (unchanged inputs):   median=${bRefreshWarm.median.toFixed(2)}ms`,
);
console.log(
  `    refreshPeakMarkers, cache MISS (peak set changed):   median=${bRefreshMiss.median.toFixed(2)}ms`,
);
console.log(
  `    refreshPeakMarkers, { skipClustering: true }:        median=${bRefreshNoCluster.median.toFixed(2)}ms`,
);
console.log(
  `    Arousal Places compute avoided by a cache hit:       ${(bRefreshMiss.median - bRefreshWarm.median).toFixed(2)}ms`,
);
console.log(
  `    Warm-cache speedup vs a cold miss:                   ${(bRefreshMiss.median / Math.max(bRefreshWarm.median, 1e-3)).toFixed(1)}x`,
);
console.log(
  `    NB: mock Leaflet — the WARM/skipClustering figures are compute-only; a real`,
);
console.log(
  `        browser adds the marker/polygon layer rebuild (see [7] for the DOM cost).`,
);
...
console.log(
  '    Problem: O(places × N) unindexed loop across all raw samples.',
);

const peaks113 = validPeaks113.map((pk) => {
  const c = track113.analyzer.getCoordinates(pk.index) || {
    lat: 51.5,
    lon: -0.1,
  };
  return {
...
const clusters113 = window.GSRSpatialClustering.compactClusters(
  peaks113,
  35,
  1.8,
);
...
const resBuildPlaces = bench(2, 10, () => {
  window.GSRArousalPlaces.buildPlaces(
    clusters113,
    scoreTracks113,
    window.GSR_CONST.AROUSAL_PLACES,
  );
});
console.log(
  `    Fixture: ${clusters113.length} clusters across ${rawPoints113.length.toLocaleString()} raw samples`,
);
console.log(
  `    Time: median=${resBuildPlaces.median.toFixed(2)}ms (min=${resBuildPlaces.min.toFixed(2)}ms, max=${resBuildPlaces.max.toFixed(2)}ms)`,
);
...
console.log(
  '    Problem: Up to 20 separate 70x70 grid KDEs + Marching Squares passes.',
);

const places113 = window.GSRArousalPlaces.buildPlaces(
  clusters113,
  scoreTracks113,
  window.GSR_CONST.AROUSAL_PLACES,
);
const meanAmp113 =
  peaks113.reduce((s, p) => s + p.amplitude, 0) / Math.max(1, peaks113.length);

const singleBlobBench = bench(3, 15, () => {
  window.GSRSpatialClustering.getConcaveBlob(
    places113[0].cluster,
    12.25,
    17.5,
    meanAmp113,
...
console.log(
  `    Single place getConcaveBlob():                   median=${singleBlobBench.median.toFixed(2)}ms`,
);
console.log(
  `    All ${places113.length} places getConcaveBlob() loop:           median=${allBlobsBench.median.toFixed(2)}ms total`,
);
...
const peaks016 = validPeaks016.map((pk) => {
  const c = track016.analyzer.getCoordinates(pk.index) || {
    lat: 51.5,
    lon: -0.1,
  };
  return {
...
const meanAmp016 =
  peaks016.reduce((s, p) => s + p.amplitude, 0) / Math.max(1, peaks016.length);

let clusters016;
const bCluster016 = bench(2, 10, () => {
  clusters016 = window.GSRSpatialClustering.compactClusters(peaks016, 35, 1.8);
});
...
const places016 = window.GSRArousalPlaces.buildPlaces(
  clusters016,
  scoreTracks016,
  window.GSR_CONST.AROUSAL_PLACES,
);
...
console.log(
  `    ${track016.analyzer.raw.length.toLocaleString()} rows, ${peaks016.length} peaks → ${clusters016.length} clusters (${places016.length} places after cap)`,
);
console.log(
  `    compactClusters():   median=${bCluster016.median.toFixed(2)}ms`,
);
console.log(
  `    buildPlaces():        median=${bBuild016.median.toFixed(2)}ms   (vs ${resBuildPlaces.median.toFixed(2)}ms on track 113)`,
);
console.log(
  `    all getConcaveBlob(): median=${bBlobs016.median.toFixed(2)}ms   (vs ${allBlobsBench.median.toFixed(2)}ms on track 113)`,
);
console.log(
  `    → full recompute ≈ ${total016.toFixed(1)}ms/frame  (track 113 ≈ ${bRefreshMiss.median.toFixed(1)}ms)`,
);
...
console.log(
  `    Track 113 (N=${peaks113.length} peaks): median=${bCompact113.median.toFixed(3)}ms`,
);
...
for (const count of [1000, 2000, 4000]) {
  ...
  while (pts.length < count) {
    pts.push(
      ...peaks113.map((p) => ({
        ...p,
        lat: p.lat + 0.002 * Math.random(),
        lon: p.lon + 0.002 * Math.random(),
      })),
    );
  }
  ...
  console.log(
    `    Collective N=${count.toString().padEnd(4)} peaks: median=${res.median.toFixed(3)}ms`,
  );
}
...
console.log(
  `    Continuous IDW (point-major splatting): median=${bIdw.median.toFixed(2)}ms`,
);
console.log(
  `    "peaks" Mode (cell-major peak scan):     median=${bPeaks.median.toFixed(2)}ms`,
);
...
console.log(
  '    Problem: peakDensity, phasicAUC, arousalIndex, triIndex allocate unpooled objects on every drag.',
);
...
console.log(
  `    Total 4-metrics compute time: median=${tMetrics.median.toFixed(2)}ms`,
);
console.log(
  `    Allocated objects per run: ~${(n113 * 4).toLocaleString()} objects (~${((n113 * 4 * 48) / (1024 * 1024)).toFixed(1)} MB heap churn per drag frame)`,
);
...
console.log(
  '    Problem: Object.keys() on GPS fixes + per-point trig in RTS backward pass.',
);
...
console.log(
  '    Problem: Thousands of L.marker instances create real DOM elements with CSS transforms.',
);
...
for (const count of [peaks113.length, 1000, 3000]) {
  ...
  const label =
    count === peaks113.length
      ? `Track 113 (${count} peaks)`
      : `Collective (${count.toString().padEnd(4)} peaks)`;
  console.log(
    `    ${label.padEnd(28)} | DOM (nodes): ${bDomMarker.median.toFixed(2)}ms | Canvas (path): ${bCircleMarker.median.toFixed(2)}ms | Speedup: ${(bDomMarker.median / Math.max(0.001, bCircleMarker.median)).toFixed(0)}x`,
  );
}
...
console.log(
  '    Problem: Moving the mouse over the graph runs full draw() (~60fps).',
);

window.AppState.analyzer = track113.analyzer;
window.AppState.viewDuration = 300;
window.AppState.viewStartTime = 0;
window.AppState.totalDuration =
  track113.analyzer.raw.length / track113.analyzer.sampleRate;
...
console.log(
  `    Full draw() execution time: median=${bFullDraw.median.toFixed(2)}ms per frame`,
);
console.log(
