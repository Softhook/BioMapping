...
function installFullscreen(window, targetEl) {
  ...
  targetEl.requestFullscreen = function () {
    requests++;
    current = targetEl;
  ...
  return {
    ...
    drop() { current = null; doc.dispatchEvent(new window.Event('fullscreenchange')); },
  };
}
...
function pressF(window) {
  window.document.dispatchEvent(new window.KeyboardEvent('keydown', { key: 'f', bubbles: true }));
}
...
test('Live display mode survives a phone lock/unlock — fullscreen is re-asserted on return', async () => {
  ...
  assert.strictEqual(fs.requests, 2, 'fullscreen re-asserted on visibility return');
...
test('a deliberate exit (browser drops fullscreen while visible) tears down display mode and is NOT resurrected by a later lock/unlock', async () => {
  ...
  assert.ok(!appContainer.classList.contains('live-display-mode'), 'display-mode chrome removed');
  ...
  setVisibility(window, 'visible');
  await wait(50);
  assert.strictEqual(fs.requests, requestsAfterExit, 'no fullscreen re-request after a genuine exit');
  assert.ok(!appContainer.classList.contains('live-display-mode'));
});
...
test('panel display mode re-asserts fullscreen after a lock/unlock', async () => {
  ...
  assert.ok(overlay.classList.contains('panel-fullscreen-overlay'), 'panel lifted into its overlay');
  ...
  assert.ok(overlay.classList.contains('display-mode'), 'display-mode class applied');
  assert.strictEqual(window.AppState.isDisplayMode, true);

  setVisibility(window, 'hidden');
  fs.drop();
  assert.ok(overlay.classList.contains('display-mode'), 'chrome kept while hidden');

  setVisibility(window, 'visible');
  assert.strictEqual(fs.requests, 2, 'overlay fullscreen re-asserted on return');
  assert.strictEqual(fs.active, true);

  await wait(200);
  assert.ok(overlay.classList.contains('display-mode'), 'display mode intact after the teardown window');
...
test('standalone FAB Full Screen is sticky across a lock/unlock', async () => {
  ...
  assert.strictEqual(fs.requests, 1, 'FAB chip entered document-root fullscreen');
  assert.strictEqual(fs.active, true);

  setVisibility(window, 'hidden');
  fs.drop();
  setVisibility(window, 'visible');
  assert.strictEqual(fs.requests, 2, 'standalone fullscreen re-asserted on return');
  assert.strictEqual(fs.active, true);

  // The FAB chip should now read "Exit Full Screen".
  const exitChip = window.document.querySelector('[data-action="exit-fullscreen"]');
...
test('standalone FAB Exit Full Screen clears the sticky target', async () => {
  ...
  window.document.querySelector('[data-action="exit-fullscreen"]')
    .dispatchEvent(new window.Event('click', { bubbles: true }));
...
test('the file-dialog "Restore Fullscreen" pill re-enters fullscreen on click', async () => {
  ...
  const pill = [...document.querySelectorAll('div')].find((d) => /Restore Fullscreen/.test(d.textContent));
  assert.ok(pill, 'restore-fullscreen pill is shown');

  pill.dispatchEvent(new window.Event('click', { bubbles: true }));
  assert.strictEqual(fs.requests, 1, 'pill click re-requests fullscreen on .app-container');
  assert.strictEqual(fs.active, true);
});
...
test('screen wake lock is re-acquired on visibility return while connected', async (t) => {
  ...
  setVisibility(window, 'visible');
  await wait(0);
  assert.strictEqual(wakeRequests, 2, 'returning to the foreground re-acquires it');

  t.after(() => { try { run("LiveState.setStatus('disconnected')"); } catch { /* torn down */ } });
});
