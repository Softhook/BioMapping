...
{
  const risingEdge = dflt.peaks.filter(
    (p) => (p.prominence || 0) < THR * 0.5 && (p.amplitude || 0) >= THR,
  );
  assert(
    risingEdge.length >= 5,
  ...
  const missedByProm = risingEdge.filter(
    (p) => !promT.some((t) => Math.abs(t - p.time) <= GAP),
  ).length;
  assert(
    missedByProm >= risingEdge.length * 0.5,
...
//    retained by the default. ──
{
  ...
  const dDef = new GSRAnalyzer();
  dDef.parseCSV(demoCsv);
  dDef.analyze({ ...D }, 0);
  const dProm = new GSRAnalyzer();
  dProm.parseCSV(demoCsv);
  dProm.analyze({ ...D, usePeakProminence: true }, 0);
...
//    before the earlier peak's apex. ──
{
  ...
  for (let i = 1; i < dflt.peaks.length; i++) {
    const a = dflt.peaks[i - 1],
      b = dflt.peaks[i];
    if (b.onsetIndex === a.onsetIndex && b.onsetIndex < a.index - 3) shared++;
  }
  assert(
    shared === 0,
    `no trailing peak shares an onset across a preceding apex (${shared})`,
  );
}

// ── Detection rule + structural invariants. ──
assert(
  dflt.peaks.every((p) => p.amplitude >= THR - 1e-9),
  `every peak's trough-to-peak amplitude >= peakThreshold (${THR})`,
);
const pv = dflt.phasic.map((d) => d.val);
assert(
  dflt.peaks.every((p) => p.onsetIndex <= p.index),
  'onset index never after the apex',
);
assert(
  dflt.peaks.every((p) => p.recoveryIndex === -1 || p.recoveryIndex > p.index),
  'recovery index, when found, after the apex',
);
assert(
  dflt.peaks.every((p) => p.amplitude > 0),
  'all amplitudes positive',
);
assert(
  dflt.peaks.every(
    (p) => typeof p.prominence === 'number' && p.prominence >= 0,
  ),
...
assert(
  keys.every((k) => k in dflt.peaks[0]),
  'peak objects carry the full field set',
);
...
for (let i = 1; i < dflt.peaks.length; i++)
  minGapIdx = Math.min(
    minGapIdx,
    dflt.peaks[i].index - dflt.peaks[i - 1].index,
  );
const minGapSamples = Math.max(1, Math.round(GAP * dflt.sampleRate));
assert(
  minGapIdx >= minGapSamples,
...
assert(
  noSnr.peaks.length >= dflt.peaks.length &&
    hiSnr.peaks.length <= dflt.peaks.length,
...
assert(
  hiQ.peaks.length < dflt.peaks.length &&
    hiQ.peaks.every((p) => p.qualityScore >= 0.7),
...
assert(
  withStaleShape.peaks.length === dflt.peaks.length,
...
assert(
  dflt.phasicDriver.length === 0 &&
    dflt.phasicClean.length === 0 &&
    dflt.phasicDriverPeaks.length === 0,
  'default path leaves deconvolution state empty',
);
assert(
  dflt._phasicOrig === null,
  'default path clears any stale pre-deconvolution phasic backup',
);
const promWins = analyze({ usePeakProminence: true });
assert(
  JSON.stringify(promWins.peaks.map((p) => p.index)) ===
    JSON.stringify(prom.peaks.map((p) => p.index)),
  'usePeakProminence runs the prominence detector, not the default',
);
const deconvWins = analyze({ useDeconvolution: true });
assert(
  deconvWins.phasicDriverPeaks.length > 0,
...
//    quirk naturally, which drifts whenever the default LPF stage changes. ──
{
  ...
  const dFull = new GSRAnalyzer();
  dFull.parseCSV(demoCsv);
  dFull.analyze({ ...D }, 0);
  const biggest = dFull.peaks.reduce((a, b) =>
    b.amplitude > a.amplitude ? b : a,
  );
  biggest.prominence = 0.01;
  const reranked = dFull._selectMemorableEvents({ ...D }, 0);
  assert(
    reranked.includes(biggest),
...
const again = analyze({});
assert(
  JSON.stringify(dflt.peaks.map((p) => [p.index, +p.amplitude.toFixed(6)])) ===
