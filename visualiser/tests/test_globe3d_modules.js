...
// rule as tests/support/load_module.js.
const G3D = (f) => {
...
function stubCesium() {
  ...
  global.Cesium = {
    ...
    EllipsoidGeometry: function (o) {
      counts.EllipsoidGeometry++;
      this._o = o;
    },
    PolygonGeometry: function (o) {
      counts.PolygonGeometry++;
      this._o = o;
    },
    WallGeometry: function (o) {
      counts.WallGeometry++;
      this._o = o;
    },
    PolygonOutlineGeometry: function (o) {
      counts.PolygonOutlineGeometry++;
      this._o = o;
    },
    PolygonHierarchy: function (p) {
      this.positions = p;
    },
    GeometryInstance: function (o) {
      counts.GeometryInstance++;
      Object.assign(this, o);
    },
    Primitive: function (o) {
      counts.Primitive++;
      this._o = o;
      this.geometryInstances = o.geometryInstances;
    },
    ...
    PerInstanceColorAppearance: function (o) {
      this._o = o;
    },
...
test('buildCzml: one extruded wall entity, heights = base + max(0,val)*scale', () => {
  ...
  assert.deepStrictEqual(
    ...
    [-0.1, 51.5, 0, -0.101, 51.501, 0, -0.102, 51.502, 0, -0.103, 51.503, 0],
  );
...
test('resolveSeries: derived field, then raw-GSR fallback for an unknown metric', () => {
  assert.deepStrictEqual(
    GSRGlobe3DExport.resolveSeries(analyzer, 'em_fog'),
    [10, 20, 30, 40],
  );
  assert.deepStrictEqual(
    GSRGlobe3DExport.resolveSeries(analyzer, 'roadClass'),
    [1, 2, 3, 4],
  );
});
...
test('GSRGlobe3DRf.buildPrimitive: one Primitive, slug count capped at MAX_SLUGS', () => {
  ...
  const prim = GSRGlobe3DRf.buildPrimitive({ raw }, drawPoints, {
    mode: 'triband',
    height: 25,
    opacity: 0.4,
  });
  ...
  assert.ok(
    counts.EllipsoidGeometry <= GSRGlobe3DRf.MAX_SLUGS &&
      counts.EllipsoidGeometry > GSRGlobe3DRf.MAX_SLUGS / 2,
...
test('GSRGlobe3DRf.buildPrimitive: slug pitch tracks distance, not sample count', () => {
  ...
  assert.ok(
    slugs > 15 && slugs < 70,
    `${slugs} evenly-spaced slugs (not ~440)`,
  );
  clearCesium();
});
...
test('GSRGlobe3DRf.buildPrimitive: null when there is nothing to draw', () => {
  ...
  assert.strictEqual(
    GSRGlobe3DRf.buildPrimitive(
      { raw: [] },
      [{ lat: 1, lon: 2, origIdx: 0 }],
      {},
    ),
    null,
  );
...
test('GSRGlobe3DRf.buildPrimitive: a dead band is squelched, matching the 2D overlay', () => {
  ...
  assert.strictEqual(
    GSRGlobe3DRf.buildPrimitive({ raw }, drawPoints, { mode: '915' }),
    null,
  ...
  const maxBlue = Math.max(
    ...prim.geometryInstances.map((gi) => gi.attributes.color.b),
  );
  assert.strictEqual(
    maxBlue,
    0,
    'dead 915 band contributes zero blue in tri-band',
  );
  clearCesium();
});
...
test('GSRGlobe3DRf.buildPrimitive: no hardware RF -> synthesises an ambient field', () => {
  ...
  assert.ok(
    prim && counts.EllipsoidGeometry > 0,
    'ambient slugs drawn even with no measured RF',
  );
  clearCesium();
});
...
const OSM_FIXTURE = {
  elements: [
    ...
    { type: 'node', id: 2, lat: 51.5, lon: -0.099 },
    { type: 'node', id: 3, lat: 51.501, lon: -0.099 },
    { type: 'node', id: 4, lat: 51.501, lon: -0.1 },
    {
      type: 'way',
      id: 10,
      nodes: [1, 2, 3, 4, 1],
...
test('GSRGlobe3DBuildings.buildPrimitive: one batched Primitive of the valid footprints only', () => {
  ...
  assert.strictEqual(
    counts.GeometryInstance,
    2,
    'only the two ≥3-node building ways',
  );
  assert.strictEqual(
    prim._o.appearance._o.translucent,
    true,
...
test('GSRGlobe3DBuildings.heightFor: explicit height > levels*3.5 > per-type > default', () => {
  ...
  assert.strictEqual(
    GSRGlobe3DBuildings.heightFor({ 'building:levels': '4' }),
    14,
  );
...
test('GSRGlobe3DBuildings.buildPrimitive: null for empty / non-building input', () => {
  ...
  assert.strictEqual(
    GSRGlobe3DBuildings.buildPrimitive({ elements: [] }, 'glass'),
    null,
  );
  clearCesium();
});
...
test('GSRGlobe3DBuildings.buildPrimitive: realistic style creates solid extruded facade body, roof caps, and outlines in a PrimitiveCollection', () => {
  ...
  assert.strictEqual(
    counts.PrimitiveCollection,
    1,
    'surface + outline primitives grouped in PrimitiveCollection',
  );
  assert.strictEqual(
    counts.Primitive,
    2,
    'one surface primitive and one outline primitive',
  );
  assert.strictEqual(
    counts.PolygonGeometry,
    4,
    'solid extruded bodies + roof caps for both buildings',
  );
  assert.strictEqual(
    counts.PolygonOutlineGeometry,
    2,
    'roof outlines created for both buildings',
  );
  clearCesium();
});
...
test('GSRGlobe3DBuildings.wallColorFor and roofColorFor assign distinct architectural palettes by tag', () => {
  ...
  const customRoof = GSRGlobe3DBuildings.roofColorFor({
    'roof:colour': '#ff0000',
  });
  ...
  assert.strictEqual(
    offRoof._css,
    '#373d47',
    'commercial/office slate charcoal cap',
  );
  assert.strictEqual(
    customRoof._css,
    '#ff0000',
    'explicit roof:colour tag respected',
  );
  ...
  const customWall = GSRGlobe3DBuildings.wallColorFor({
    'building:colour': '#00ff00',
  });
  ...
  assert.strictEqual(
    customWall._css,
    '#00ff00',
    'explicit building:colour tag respected',
  );
  clearCesium();
});
...
test('GSRGlobe3DBuildings.tileStyleExpression covers every style', () => {
  ...
  for (const s of ['glass', 'dark', 'monochrome', 'realistic']) {
    ...
    assert.ok(
      expr.includes('color('),
      `style ${s} should produce a valid Cesium color expression`,
    );
  }
  clearCesium();
});
