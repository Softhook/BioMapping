...
({ CVXEDA: global.CVXEDA } = require(
  path.join(__dirname, '../src/signal/cvxeda.mjs'),
));
loadModule(
...
assert(
  lrConst.every((v) => Math.abs(v - 2.5) < 1e-6),
  'LR4 passes DC with exact unity gain (0 dB error)',
);

// Biological SCR frequency (0.2 Hz)
const tone02Hz = Array.from({ length: n }, (_, i) =>
  Math.sin((2 * Math.PI * 0.2 * i) / sr),
);
...
assert(
  passbandRatio > 0.95,
  `LR4 preserves >95% amplitude at 0.2 Hz SCR passband (got ${(passbandRatio * 100).toFixed(1)}%)`,
);

// Cadence resonance frequency (1.8 Hz, Track 24 footstep tremor)
const tone18Hz = Array.from({ length: n }, (_, i) =>
  Math.sin((2 * Math.PI * 1.8 * i) / sr),
);
const filtered18Hz = GsrFilter.applyZeroPhaseLinkwitzRiley(tone18Hz, 1.0, sr);
const gaitRmsRatio =
  rms(filtered18Hz.slice(100, 900)) / rms(tone18Hz.slice(100, 900));
assert(
  gaitRmsRatio < 0.05,
  `LR4 attenuates >95% amplitude at 1.8 Hz footstep cadence (residual: ${(gaitRmsRatio * 100).toFixed(2)}%)`,
);

// High-frequency noise (3.0 Hz)
const tone30Hz = Array.from({ length: n }, (_, i) =>
  Math.sin((2 * Math.PI * 3.0 * i) / sr),
);
const filtered30Hz = GsrFilter.applyZeroPhaseLinkwitzRiley(tone30Hz, 1.0, sr);
const hfRmsRatio =
  rms(filtered30Hz.slice(100, 900)) / rms(tone30Hz.slice(100, 900));
assert(
  hfRmsRatio < 0.01,
  `LR4 attenuates >99% amplitude at 3.0 Hz (residual: ${(hfRmsRatio * 100).toFixed(3)}%)`,
);
...
const centerIdx = 100;
for (let i = 0; i < 201; i++) {
  pulse[i] = Math.exp(-0.5 * ((i - centerIdx) / 10) ** 2);
}
...
assert(
  maxIdx === centerIdx,
  `LR4 is zero-phase: pulse peak remains exactly at index ${centerIdx}`,
);
...
assert(
  maxStepVal < maxButter4Val,
  `LR4 has lower transient peaking than Butterworth o4 (${maxStepVal.toFixed(4)} vs ${maxButter4Val.toFixed(4)})`,
);
assert(
  maxStepVal <= 1.05,
  `LR4 transient overshoot <= 5% (${maxStepVal.toFixed(4)})`,
);
...
function generateScr(amp, tauSlow, tauFast, durationSec, fs) {
  ...
  const tPeak =
    ((tauSlow * tauFast) / (tauSlow - tauFast)) * Math.log(tauSlow / tauFast);
  const normFactor =
    1 / (Math.exp(-tPeak / tauSlow) - Math.exp(-tPeak / tauFast));
  const onset = Math.round(5 * fs); // onset at 5s
  for (let i = onset; i < len; i++) {
    const t = (i - onset) / fs;
    out[i] =
      amp * normFactor * (Math.exp(-t / tauSlow) - Math.exp(-t / tauFast));
  }
  return out;
}
...
assert(
  lrRetention > 0.95,
  `LR4 preserves >95% peak amplitude on genuine SCR (got ${(lrRetention * 100).toFixed(1)}%)`,
);
assert(
  lrRetention > box05Retention,
  `LR4 preserves higher amplitude than box 0.5s (${(lrRetention * 100).toFixed(1)}% vs ${(box05Retention * 100).toFixed(1)}%)`,
);
assert(
  box11Retention < 0.8,
  `Box 1.1s severely attenuates SCR amplitude (<80%, got ${(box11Retention * 100).toFixed(1)}%)`,
);
...
if (track24Available) {
  ...
  assert(
    analyzerProd.filtered !== null && analyzerProd.filtered.length > 0,
    'Production analyzer generates filtered array',
  );
  assert(
    analyzerProd.filtered.length === analyzerProd.raw.length,
    'filtered matches raw length',
  );
  assert(
    analyzerProd.peaks.length === 211,
    `Production pipeline detects exactly 211 peaks on Track 24 (got ${analyzerProd.peaks.length})`,
  );

  const meanQuality =
    analyzerProd.peaks.reduce((s, p) => s + p.qualityScore, 0) /
    analyzerProd.peaks.length;
  assert(
    meanQuality >= 0.76,
    `Mean peak quality >= 0.760 (got ${meanQuality.toFixed(3)})`,
  );

  const sumAmp = analyzerProd.peaks.reduce((s, p) => s + p.amplitude, 0);
  assert(
    sumAmp >= 24.0,
    `Summed amplitude >= 24.0 uS (got ${sumAmp.toFixed(1)} uS)`,
  );
  ...
  assertClose(
    ...
    4.56,
    0.1,
    'Max peak amplitude is approximately 4.56 uS',
  );
  ...
  assert(
    analyzerRaw.peaks.length === 337,
    `Raw signal produces 337 peaks due to footstep ripple (got ${analyzerRaw.peaks.length})`,
  );
  assert(
    analyzerRaw.peaks.length - analyzerProd.peaks.length === 126,
  ...
  assert(
    ampLossPct > 0.25,
    `Box 1.1s destroys >25% signal amplitude on Track 24 (loss: ${(ampLossPct * 100).toFixed(1)}%)`,
  );
  ...
  assert(
    analyzerProm.peaks.length > 0,
    'Prominence detector detects peaks on Track 24',
  );
  assert(
    analyzerProm.peaks.every((p) => p.prominence >= D.peakThreshold),
    'Every prominence peak satisfies peakThreshold',
  );
  ...
  assert(
    aucCurve.length === analyzerProd.phasic.length,
    'Phasic AUC curve matches phasic length',
  );
  assert(
    aucCurve.every((d) => !isNaN(d.val) && d.val >= 0),
    'Phasic AUC contains non-negative finite numbers',
  );
} else {
...
for (let i = 0; i < scr1.length && i + 100 < synthLen; i++)
  synthSignal[i + 100] += scr1[i];
for (let i = 0; i < scr2.length && i + 350 < synthLen; i++)
  synthSignal[i + 350] += scr2[i];
...
for (let i = 0; i < synthLen; i++) {
  csvRows.push(
    `${(i / sr).toFixed(2)},${synthSignal[i].toFixed(4)},51.5074,-0.1278,1.3`,
  );
}
...
assert(
  analyzerCvx.phasicDriver !== null,
  'cvxEDA generates sparse phasicDriver',
);
assert(
  analyzerCvx.phasic.length === synthLen,
  'cvxEDA produces valid phasic reconstruction',
);
assert(analyzerCvx.tonic.length === synthLen, 'cvxEDA produces B-spline tonic');
assert(
  analyzerCvx.peaks.length >= 2,
  `cvxEDA identifies injected SCR peaks (got ${analyzerCvx.peaks.length})`,
);

// Driver sparsity: the majority of samples in the driver must be exactly zero
const zeroDriverCount = analyzerCvx.phasicDriver.filter(
  (d) => Math.abs(d.val) < 1e-4,
).length;
const driverSparsity = zeroDriverCount / analyzerCvx.phasicDriver.length;
assert(
  driverSparsity > 0.85,
  `cvxEDA driver is sparse (>85% zeros, got ${(driverSparsity * 100).toFixed(1)}%)`,
);
