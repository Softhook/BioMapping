...
loadModule(path.join(__dirname, '../src/signal/stats_math.mjs')); // also read directly by ui.js
...
test('GSRUI.correlationBand: |r| bands are negligible <.10, small <.20, moderate <.30, strong otherwise (sign-independent)', () => {
  ...
  assert.strictEqual(band(-0.099), 'negligible');
  assert.strictEqual(band(0.1), 'small');
  assert.strictEqual(band(-0.19), 'small');
  assert.strictEqual(band(0.2), 'moderate');
  assert.strictEqual(band(-0.29), 'moderate');
  assert.strictEqual(band(0.3), 'strong');
...
test('updateEnvironmentalDashboard (single mode): cache reused across repeated calls when nothing changed', () => {
  ...
  assert.ok(
    first.allData.length > 0,
    'sanity: dashboard aggregated real data points from the fixture',
  );
  ...
  assert.strictEqual(
    a._cachedEnvStats,
    first,
    'no mutation occurred -> cache object reused, not recomputed',
  );
});
...
test('updateEnvironmentalDashboard (single mode): EM Fog is included as a correlation feature when the track carries readings', () => {
  ...
  assert.ok(
    Number.isFinite(emRow.rPhasic) && Number.isFinite(emRow.pPhasic),
  ...
  GSRUI.updateEnvironmentalDashboard();
  assert.ok(
    !b._cachedEnvStats.correlationMatrix.some((r) => r.key === 'em_fog'),
...
test('updateEnvironmentalDashboard (single mode): correlation matrix carries FDR q-values, "in park" is a feature, and road profile drops "unclassified"', () => {
  ...
  a.raw.forEach((pt, i) => {
    pt.osm_road_class = ['residential', 'unclassified', 'primary'][i % 3];
  });
  Object.assign(RealAppState, {
    viewMode: 'single',
    analyzer: a,
  ...
  stats.correlationMatrix.forEach((row) => {
    assert.ok(
      'qPhasic' in row && 'qTonic' in row && 'qPeaks' in row,
  ...
  assert.ok(
    stats.correlationMatrix.some((r) => r.key === 'osm_in_park'),
  ...
  assert.ok(
    stats.roadComparison && Number.isFinite(stats.roadComparison.p),
    'roadComparison holds a finite Welch p-value',
  );
  assert.ok(
    !('_phasicVals' in stats.roadProfile[0]),
...
test('updateEnvironmentalDashboard (single mode): the highest-vs-lowest road gap is Bonferroni-corrected for being the widest of N classes', () => {
  ...
  assert.ok(
    rc.pAdj >= rc.p - 1e-12,
    `selection-corrected pAdj (${rc.pAdj}) is not smaller than the raw p (${rc.p})`,
  );
...
test('updateEnvironmentalDashboard (single mode): a factor that never changes along the route is flagged as no-variance and left unstarred', () => {
  ...
  GSRUI.updateEnvironmentalDashboard();
  const row = a._cachedEnvStats.correlationMatrix.find(
    (r) => r.key === 'osm_green_pct_50m',
  );
  assert.ok(
    row && row.hasVariance === false,
...
test('updateEnvironmentalDashboard (single mode): recomputes after setPeakLabel edits the active track', () => {
  ...
  assert.ok(
    a.peaks.length > 0,
    'sanity: fixture produces at least one peak under default params',
  );
  Object.assign(RealAppState, {
    viewMode: 'single',
    analyzer: a,
  ...
  assert.notStrictEqual(
    after,
    before,
    'setPeakLabel bumped _dataVersion -> dashboard cache recomputed, not served stale',
  );
});
...
test('updateEnvironmentalDashboard (single mode): recomputes after setPeakExcluded toggles a peak', () => {
  ...
  assert.notStrictEqual(
    after,
    before,
    'setPeakExcluded bumped _dataVersion -> dashboard cache recomputed',
  );
});
...
test('updateEnvironmentalDashboard (single mode): recomputes after the active track is re-analyzed', () => {
  ...
  assert.notStrictEqual(
    after,
    before,
    're-running analyze() bumped _dataVersion -> dashboard cache recomputed',
  );
});
...
test('updateEnvironmentalDashboard (collective mode): method scales with walk count (single / fewWalks / metaProvisional / meta)', () => {
  ...
  six.forEach((r) => {
    ...
    assert.ok(
      Number.isFinite(r.pPhasic) && r.pPhasic >= 0 && r.pPhasic <= 1,
      `${r.key}: valid meta p`,
    );
...
test('updateEnvironmentalDashboard: tonic gets its own longer-lag environment, Peaks is aggregated to 15 s bins, FDR is per channel', () => {
  ...
  stats.correlationMatrix
    ...
    .forEach((r) => {
      assert.ok(
        r.kPeaks <= r.kPhasic,
        `${r.key}: kPeaks (${r.kPeaks}) <= kPhasic (${r.kPhasic})`,
      );
    });
  ...
  stats.correlationMatrix.forEach((r, i) => {
    if (Number.isFinite(expectedQ[i])) {
      assert.ok(
        Math.abs(r.qPhasic - expectedQ[i]) < 1e-12,
...
test('updateEnvironmentalDashboard: the road profile groups tonic arousal by the tonic-lagged road class, not the phasic-lagged one', () => {
  ...
  assert.ok(
    service.timeSpent >= 10 && primary.timeSpent >= 10,
...
test('updateEnvironmentalDashboard (collective mode): only enriched tracks are analysed; un-enriched ones are ignored', () => {
  ...
  assert.strictEqual(
    stats.trackCount,
    3,
    'only the 3 enriched tracks feed the analysis',
  );
  // The bare track's samples must not appear in allData.
  assert.ok(
    stats.allData.every((d) => d.trackId !== 'trkBare'),
    'un-enriched track contributes no samples',
  );
});
...
test('updateEnvironmentalDashboard (single mode): correlation cells use the single-recording method', () => {
  ...
  GSRUI.updateEnvironmentalDashboard();
  a._cachedEnvStats.correlationMatrix
    .filter((r) => r.hasVariance)
    .forEach((r) => {
      assert.strictEqual(
        r.mPhasic,
        'single',
        `${r.key}: single walk -> 'single' method`,
      );
...
test('updateEnvironmentalDashboard (collective mode): mutating ONE of several active tracks still invalidates the shared collective cache', () => {
  ...
  GSRUI.updateEnvironmentalDashboard();
  assert.strictEqual(
    collectiveManager._cachedEnvStats,
    first,
    'no mutation -> cache reused',
  );
  ...
  assert.notStrictEqual(
    after,
    first,
    'mutating track B alone still invalidates the joined collective cache',
  );
});
...
test('renderCorrelationTable: interpretation distinguishes "suggestive" (raw p<.05, q≥.05) from negligible and from significant', () => {
  ...
  global.document = {
    querySelector: (s) =>
      s.includes('correlationTable')
        ? {
            set innerHTML(_) {},
            appendChild(el) {
              cells.push(el.__html);
            },
          }
        : null,
    getElementById: (id) =>
      id === 'correlationMethodNote'
        ? {
            set innerHTML(v) {
              noteSink.v = v;
            },
  ...
  try {
    ...
    assert.match(
      interp[0],
      /^Suggestive small link to lower baseline arousal .*p = 0\.020 before correction/,
    );
    assert.strictEqual(
      interp[1],
      'No link to arousal — effect sizes are negligible',
    );
    ...
    assert.ok(
      tdCount >= 4,
      `row should have the name + 3 q columns + interpretation, got ${tdCount} plain <td>`,
    );
  } finally {
...
test('updateEnvironmentalDashboard (single mode): computes speed-adjusted partial correlation on tonic channel', () => {
  ...
  assert.ok(
    stats.allData.some((d) => typeof d.speed === 'number' && d.speed > 0),
    'allData contains positive walking speed in m/s',
  );

  const row = stats.correlationMatrix.find(
    (r) => r.key === 'osm_green_pct_50m',
  );
  assert.ok(row, 'osm_green_pct_50m row present');
  assert.ok(
    'rTonicSpeedAdj' in row,
    'rTonicSpeedAdj is present on correlation row',
  );
  assert.ok(
    Number.isFinite(row.rTonicSpeedAdj),
    `rTonicSpeedAdj is a finite number, got ${row.rTonicSpeedAdj}`,
  );
  assert.ok(
    'pTonicSpeedAdj' in row,
    'pTonicSpeedAdj is present on correlation row',
  );
});
...
test('sortRoadArousalTable: toggles sort direction and sorts road profile rows', () => {
  ...
  global.document = {
    querySelector: (s) =>
      s.includes('roadArousalTable')
        ? {
            set innerHTML(_) {},
            appendChild(el) {
              renderedRoads.push(el.__html);
            },
          }
        : null,
    getElementById: (id) => {
      if (id === 'roadBarChartContainer')
        return {
          set innerHTML(_) {},
          appendChild(el) {
            renderedBars.push(el.__html);
          },
        };
