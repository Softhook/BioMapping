...
for (const col of CSV_COLUMNS) {
  ...
  if (col === 'timestamp') {
    assert(
      'time' in firstPoint,
      `Canonical field 'timestamp' mapped to 'time'`,
    );
  } else if (col === 'gsr_raw') {
    assert('val' in firstPoint, `Canonical field 'gsr_raw' mapped to 'val'`);
  } else if (col === 'speed_kts') {
    assert(
      'speedKts' in firstPoint,
      `Canonical field 'speed_kts' mapped to 'speedKts'`,
    );
  } else if (col === 'course_deg') {
    assert(
      'course' in firstPoint,
      `Canonical field 'course_deg' mapped to 'course'`,
    );
  } else if (col === 'fix_type') {
    assert(
      'fixType' in firstPoint,
      `Canonical field 'fix_type' mapped to 'fixType'`,
    );
  } else if (col === 'hacc_m') {
    ...
    assert(
      col in firstPoint,
      `Canonical field '${col}' present on parsed point`,
    );
  }
}
...
assert(
  isNaN(firstPoint.hacc),
  'hacc is NaN when the source CSV predates the hacc_m column (v1.1 fixture)',
);

// Count GPS fixes
const gpsFixes = raw.filter(
  (p) => p._isGpsFix && !isNaN(p.lat) && !isNaN(p.lon),
);
...
if (hasHdop) {
  ...
  assert(
    gpsPoints.length <= initialCount,
    `HDOP gate: ${initialCount} → ${gpsPoints.length}`,
  );
} else {
...
assert(
  ...
  `Filtered GPS has valid points: ${validFiltered.length}`,
);
console.log(
...
assertEq(
  drawDownsampled.length,
  drawPoints.length,
  'No-downsample preserves count',
);
...
assert(
  pathRmsM > 0.01,
  `Reconstructed paths differ: RMS = ${pathRmsM.toFixed(2)} m (cache invalidation works)`,
);
console.log(
  `  Reconstructed path difference: ${pathRmsM.toFixed(2)} m RMS (cache correctly invalidated)`,
);

// Also verify the cache key itself changed
assert(
  mockAnalyzerA._filteredGpsCacheKey !== mockAnalyzerB._filteredGpsCacheKey,
...
for (let i = 0; i < Math.min(10, kalmanResult.length); i++) {
  ...
  assert(
    dist < 50,
    `Kalman point ${i} within 50 m of raw (${dist.toFixed(1)} m)`,
  );
}

// Verify sample rate detection
assert(
  analyzer.sampleRate > 5 && analyzer.sampleRate < 20,
