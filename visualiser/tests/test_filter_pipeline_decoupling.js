...
loadModule(path.join(__dirname, '../src/signal/analyzer_time_format.mjs'));
...
test('decoupled filter pipeline: Gait Filter ON + lpfWindow 0 retains pure LR4 output', () => {
  ...
  const expectedLR4 = GsrFilter.applyZeroPhaseLinkwitzRiley(
    rawVals,
    1.0,
    a.sampleRate,
  );
  ...
  for (let i = 0; i < 50; i++) {
    assert.ok(
      Math.abs(filteredGaitOnly[i] - expectedLR4[i]) < 1e-9,
      `mismatch at ${i}: ${filteredGaitOnly[i]} vs ${expectedLR4[i]}`,
    );
  }
});
...
test('decoupled filter pipeline: Gait Filter ON + lpfWindow > 0 cascades smoothing (no longer ignored)', () => {
  ...
  assert.ok(
    diffCount > 0,
    `lpfWindow > 0 should actively smooth the signal in cascade with Gait Filter (found ${diffCount} differences)`,
  );
});
...
test('decoupled filter pipeline: Gait Filter OFF + lpfWindow > 0 applies standalone Butterworth smoothing', () => {
  ...
  const rawVals = a.raw.map((d) => d.val);
  const expectedBW = GsrFilter.applyZeroPhaseButterworth(
    rawVals,
    3.0,
    4,
    a.sampleRate,
  ...
  for (let i = 0; i < 50; i++) {
    assert.ok(
      Math.abs(filteredBW[i] - expectedBW[i]) < 1e-9,
      `mismatch at ${i}`,
    );
  }
});
...
test('decoupled filter pipeline: Gait Filter ON and Butterworth can be active at the same time', () => {
  ...
  const rawVals = a.raw.map((d) => d.val);
  const expectedSmooth = GsrFilter.applyZeroPhaseButterworth(
    rawVals,
    3.0,
    4,
    a.sampleRate,
  );
  const expectedCascaded = GsrFilter.applyZeroPhaseLinkwitzRiley(
    expectedSmooth,
    1.0,
    a.sampleRate,
  );
  ...
  for (let i = 0; i < 50; i++) {
    assert.ok(
      Math.abs(filteredCascaded[i] - expectedCascaded[i]) < 1e-9,
      `mismatch at ${i}`,
    );
  }
});
...
test('decoupled filter pipeline: Hampel artifact removal cleans spike before low-pass stage', () => {
  ...
  assert.ok(
    filtered[20] < 10.0,
    `Spike of 50.0 was cleaned down to ${filtered[20]}`,
  );
});
