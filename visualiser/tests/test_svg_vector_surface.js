...
assert(
  surfaceLayers && typeof surfaceLayers === 'object',
  '_surface returns layer object',
);
assert(Array.isArray(surfaceLayers.mesh), 'surfaceLayers.mesh is an array');
assert(
  surfaceLayers.mesh.length === 9,
  `mesh contains 9 cell polygons (got ${surfaceLayers.mesh.length})`,
);
assert(
  surfaceLayers.mesh[0].includes('fill="#'),
  'mesh elements use standard Hex #RRGGBB colors for Illustrator compatibility',
);
assert(
  surfaceLayers.isobands[0].includes('fill="#') ||
    surfaceLayers.isobands[0].includes('stroke="#'),
  'isoband elements use standard Hex #RRGGBB colors for Illustrator compatibility',
);

console.log(
  '✓ _surface correctly generates vector mesh polygons and vector isobands with standard Hex colors',
);
...
assert(
  greenIsobandFound,
  'Vector_Surface_Isobands layer contains valid green contour isoline elements',
);
console.log(
  '✓ Vector_Surface_Isobands layer verified to contain green contour isoline paths',
);
...
assert(
  !isNaN(smoothedPts[0].lon) && !isNaN(smoothedPts[0].lng),
  'GeoUtils.chaikinSmooth supports Leaflet {lat, lng} objects without producing NaN',
);
const smoothPathD = GSRMapExporter._pathD(mockMap, sampleTrackLatLngs, false);
assert(
  smoothPathD.includes('C'),
  '_pathD generates smooth cubic Bézier C curve commands instead of jaggity L commands',
);
console.log(
  '✓ _pathD generates smooth cubic Bézier spline strokes for track paths',
);
...
assert(
  svgOutput.includes('id="Vector_Surface_Mesh"'),
  'SVG contains Vector_Surface_Mesh layer group',
);
assert(
  svgOutput.includes('data-name="Vector Surface Mesh"'),
  'SVG contains Vector Surface Mesh layer name',
);
assert(
  svgOutput.includes('id="Vector_Surface_Isobands"'),
  'SVG contains Vector_Surface_Isobands layer group',
);
assert(
  svgOutput.includes('data-name="Vector Surface Isobands"'),
  'SVG contains Vector Surface Isobands layer name',
);
assert(
  !svgOutput.includes('id="Raster_Surface_Fallback"'),
  'SVG contains NO Raster_Surface_Fallback layer group',
);
...
assert(
  !meshSection.includes('<image'),
  'Vector_Surface_Mesh Section contains NO raster image elements',
);

console.log(
  '✓ SVG output successfully includes separate Vector_Surface_Mesh and Vector_Surface_Isobands layers without raster fallback',
);
...
assert(
  fallbackLayers.mesh.length === 0,
  'No vector mesh when surfaceData is missing',
);
assert(
  fallbackLayers.isobands.length === 0,
  'No vector isobands when surfaceData is missing',
);
...
assert.strictEqual(
  projInfo.w,
  2000,
  '_getProjection initializes targetW = 2000 for zoom-independent export',
);
assert(
  projInfo.h >= 800,
  '_getProjection initializes targetH proportional to latitude/longitude span',
);
...
assert.strictEqual(
  projPt1.x.toFixed(3),
  projPt2.x.toFixed(3),
  'projInfo.project handles both array and lat/lon object inputs identically',
);
console.log(
  '✓ _getProjection constructs 2000px dataset-framed Mercator projection independently of screen zoom',
);
...
assert(
  isolineD.includes('C'),
  'Open external isoline is rendered with smooth cubic Bézier C spline commands',
);
assert(
  !isolineD.includes('0.000 0.000') && !isolineD.includes('2000.000 2000.000'),
  'Isoband d string contains NO 4-corner map extent bounding box anchors',
);
console.log(
  '✓ Open external isolines generate smooth curved splines with zero 4-corner map extent anchors',
);
...
function assertContourSmoothness(dString, maxAllowedAngleDeg = 30) {
  ...
  assert(
    segments.length >= 2,
    'Contour path contains multiple cubic Bézier segments for smoothness analysis',
  );
...
console.log(
  '✓ MATHEMATICALLY PROVED: Contour path has smooth tangent continuity (C1) with zero sharp or jagged corners',
);
...
// ── Regression Test: Coordinate Array Parsing in _pathD ──────────────────────
{
  ...
  assert.strictEqual(
    exactD,
    'M100.000 500.000 L110.000 510.000 L120.000 520.000',
    'Exact mode with coordinate arrays parsed correctly',
  );

  // Smooth mode
  const smoothD = GSRMapExporter._pathD(project, pathData, false, true, false);
  assert(
    smoothD.startsWith('M100.0'),
    'Smooth mode starts at correct coordinates',
  );
  ...
  const nestedD = GSRMapExporter._pathD(
    project,
    nestedPaths,
    false,
    false,
  ...
  assert.strictEqual(
    nestedD,
    'M100.000 500.000 L101.000 501.000 M102.000 502.000 L103.000 503.000',
    'Nested paths are correctly joined',
  );

  console.log(
    '✓ Regression Test: Coordinate arrays parsed correctly in both exact/smooth/nested modes without falling back to (0,0)',
  );
}
