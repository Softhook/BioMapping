...
function importFreshNotices() {
  const url = pathToFileURL(
    path.join(__dirname, '../src/core/notices.mjs'),
  ).href;
...
test('GSRNotices.report: creates a visible, non-blocking toast when a DOM exists', () => {
  ...
  const makeNode = () => ({
    ...
    id: '',
    remove() {
      bodyChildren = bodyChildren.filter((n) => n !== this);
    },
    appendChild(c) {
  ...
  assert.strictEqual(
    bodyChildren.length,
    1,
    'a toast container is appended to body',
  );
  ...
  assert.ok(
    toast.textContent.includes('surfaced'),
    'toast shows the error message',
  );
  assert.ok(
    toast.textContent.includes('test-context'),
    'toast shows the reporting context',
  );
  assert.strictEqual(
    toast.style.background,
    '#7f1d1d',
    'toast uses the error styling',
  );
...
test('GSRNotices.report: reuses the existing container instead of stacking new ones', () => {
  ...
  assert.strictEqual(
    bodyChildren.length,
    0,
    'no new container appended — the existing one is reused',
  );
  assert.strictEqual(
    container.children.length,
    2,
    'both toasts were added to the shared container',
  );
...
test('GSRNotices: window error/unhandledrejection hooks surface uncaught errors', async () => {
  ...
  try {
    ...
    assert.strictEqual(
      typeof listeners.error,
      'function',
      'window error hook registered',
    );
    assert.strictEqual(
      typeof listeners.unhandledrejection,
      'function',
      'unhandledrejection hook registered',
    );
    ...
    assert.strictEqual(
      bodyChildren.length,
      1,
      'toast container appears on uncaught error',
    );
    const toast = bodyChildren[0].children[0];
    assert.ok(
      toast.textContent.includes('uncaught boom'),
      'toast surfaces the uncaught error message',
    );
  } finally {
...
test('GSRNotices: benign ResizeObserver loop diagnostic does not spawn a red toast', async () => {
  ...
  try {
    ...
    assert.strictEqual(
      bodyChildren.length,
      0,
      'benign ResizeObserver diagnostic must not create a toast',
    );
    ...
    assert.strictEqual(
      bodyChildren.length,
      1,
      'a real error still toasts after the benign one was swallowed',
    );
  } finally {
    delete global.window;
    delete global.document;
...
test('GSRNotices.warn: shows an amber toast (not the red error toast)', () => {
  ...
  const toast = bodyChildren[0].children[0];
  assert.ok(
    toast.textContent.includes('careful now'),
    'toast shows the warning message',
  );
  assert.ok(
    toast.textContent.includes('warn-ctx'),
    'toast shows the warning context',
  );
  assert.strictEqual(
    toast.style.background,
    '#92400e',
    'warning uses amber styling, distinct from the red error toast',
  );
...
test('GSRNotices.warn: { toast: false } logs only and creates no toast', () => {
  ...
  assert.strictEqual(
    bodyChildren.length,
    0,
    'no toast created when toast:false (e.g. a modal already shows the warning)',
  );
  delete global.document;
});
...
test('GSRNotices.dialog: resolves with the clicked button value and removes the overlay', async () => {
  ...
  try {
    ...
    assert.strictEqual(
      buttons.length,
      3,
      'two action buttons plus a Cancel button',
    );
    assert.ok(
      dom.window.document.body.textContent.includes('Unsaved Labels'),
      'dialog title rendered',
    );
    assert.ok(
      dom.window.document.body.textContent.includes('unsaved peak labels'),
    ...
    assert.strictEqual(
      action,
      'export',
      'resolves with the clicked button value',
    );
    assert.strictEqual(
      dom.window.document.querySelectorAll('body *').length,
      0,
      'overlay is removed after choosing',
    );
  } finally {
    delete global.document;
    dom.window.close();
...
test('GSRNotices.dialog: resolves null on overlay (outside) click', async () => {
  ...
  try {
    ...
    assert.strictEqual(
      await p,
      null,
      'clicking outside the card dismisses with null',
    );
  } finally {
    delete global.document;
    dom.window.close();
...
test('GSRNotices.dialog: is announced as a dialog and moves focus to its first button', async () => {
  ...
  try {
    ...
    const titleEl = dom.window.document.getElementById(
      card.getAttribute('aria-labelledby'),
    );
    assert.ok(
      titleEl && titleEl.textContent === 'Replace Tracks',
      'aria-labelledby points at the title',
    );
    const firstButton = dom.window.document.querySelector(
      '[role="dialog"] button',
    );
    assert.strictEqual(
      dom.window.document.activeElement,
      firstButton,
      'focus moves into the dialog, on the first button',
    );
    firstButton.click();
    await p;
  } finally {
...
test('GSRNotices.dialog: restores focus to the trigger when it closes', async () => {
  const dom = new JSDOM(
    '<!doctype html><body><button id="trigger">Open</button></body>',
  );
  global.document = dom.window.document;
  try {
    ...
    assert.notStrictEqual(
      dom.window.document.activeElement,
      trigger,
      'focus leaves the trigger while the dialog is open',
    );
...
test('GSRNotices.escapeHtml: safely escapes &, <, >, " and handles null/undefined/numbers', () => {
  assert.strictEqual(
    GSRNotices.escapeHtml('hello & world'),
    'hello &amp; world',
  );
  assert.strictEqual(
    GSRNotices.escapeHtml('<script>alert("xss")</script>'),
