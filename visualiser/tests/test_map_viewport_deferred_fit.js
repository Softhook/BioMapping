...
function makeFakeMap(size) {
  ...
  return {
    ...
    flyToBounds(b, o) {
      calls.flyToBounds.push({ b, o });
    },
    fitBounds(b, o) {
      calls.fitBounds.push({ b, o });
    },
  };
}
...
test('_flyOrFitBounds: hidden map (size 0) defers instead of throwing', () => {
  ...
  assert.strictEqual(
    mm.map.calls.flyToBounds.length,
    0,
    'Leaflet is never called on a zero-size map',
  );
...
test('_applyPendingFit: replays the deferred fit once the map has a size', () => {
  ...
  assert.strictEqual(
    mm.map.calls.flyToBounds.length,
    0,
    'replayed as a static fit, not a fly',
  );
  assert.strictEqual(mm.map.calls.fitBounds.length, 1);
  assert.strictEqual(mm.map.calls.fitBounds[0].b, bounds);
  assert.strictEqual(
    mm.map.calls.fitBounds[0].o.maxZoom,
    15,
    'the original opts are carried through',
  );
