...
// 4h. applyKalman — identity (no change with balanced Q/R on straight line)
{
  ...
  assert(
    rmsFromInput < 15.0,
    `Kalman tracks input: RMS from input = ${rmsFromInput.toFixed(2)} m`,
  );
}
...
// 4j2. applyKalman — uses hacc_m directly instead of DOP²-scaling when present
{
  ...
  let rmsNoHacc = 0,
    rmsWithHacc = 0;
  for (let i = 0; i < 30; i++) {
    const dNo = GeoUtils.haversineMeters(
      resNoHacc[i].lat,
      resNoHacc[i].lon,
      noHacc[i].lat,
      noHacc[i].lon,
    );
    const dWith = GeoUtils.haversineMeters(
      resWithHacc[i].lat,
      resWithHacc[i].lon,
      withHacc[i].lat,
      withHacc[i].lon,
...
// 4j3. applyKalman — hacc_m sentinel (99.9 = unknown) falls back to DOP scaling
{
  ...
  for (let i = 0; i < ptsSentinel.length; i++) {
    assert(
      Math.abs(resultSentinel[i].lat - resultNoHacc[i].lat) < 1e-12,
...
// 4q. applyKalman — RTS displacement clamp constraint
{
  ...
  assert(
    dist <= maxDispAllowed + 0.1,
    `Kalman RTS clamp limits deviation to 3σ: dist=${dist.toFixed(2)} m <= max=${maxDispAllowed} m`,
  );
}
...
// 4m. applyVelocitySmoothing — blends with dead reckoning
{
  ...
  assert(
    !isNaN(result[1].lat) && !isNaN(result[1].lon),
    'VelocitySmoothing output is valid',
  );
}
...
// 4m2. applyVelocitySmoothing — prefers hacc_m over HDOP for trust weighting
{
  ...
  const distNoHacc = GeoUtils.haversineMeters(
    noHacc[1].lat,
    noHacc[1].lon,
    rawFix.lat,
    rawFix.lon,
  );
  const distBadHacc = GeoUtils.haversineMeters(
    badHacc[1].lat,
    badHacc[1].lon,
    rawFix.lat,
    rawFix.lon,
...
// 4n. applyStopAveraging — collapses stationary cluster
{
  ...
  assertEq(
    result.length,
    5,
    'StopAveraging preserves count (locks to centroid)',
  );
  ...
  for (let i = 1; i < 4; i++) {
    assertClose(
      ...
      centLat,
      1e-10,
      `StopAveraging cluster[${i}] locked to centroid`,
    );
  }
...
{
  const pts = [
    { lat: 0, lon: 0, time: 0, speedKts: 5, hdop: 1, course: 90 },
