...
test('queryBBoxRaw: finds an item whose exact bbox overlaps the query bbox', () => {
  ...
  const item = { id: 'a' };
  grid.insert(bboxAround(51.5, -0.1), item);

  const found = grid.queryBBoxRaw(bboxAround(51.5, -0.1, 0.001));
  assert.ok(
    found.includes(item),
    'item is returned when the query bbox overlaps its own cell',
  );
});
...
test('queryBBoxRaw: excludes an item whose cell range does not overlap the query bbox', () => {
  ...
  const found = grid.queryBBoxRaw(bboxAround(52.5, -1.1, 0.001)); // ~1 degree away
  assert.ok(
    !found.includes(item),
    'item is excluded when nowhere near the query bbox',
  );
});
...
test('queryBBoxRaw: with no insert-side padding, an item one cell away from the query point is not found', () => {
  ...
  assert.ok(
    !found.includes(item),
    "unpadded insert does not widen an item's catchment beyond its own cell range — mirrors rf_fluid_renderer.js's exact-bbox semantics",
  );
});
...
test("insert: paddingCells widens an item's catchment area beyond its own cell range", () => {
  ...
  assert.ok(
    found.includes(item),
    'paddingCells=1 makes the item reachable from an adjacent cell',
  );
});
...
test('insert: paddingCells=0 (default) does not widen the catchment area', () => {
  ...
  assert.ok(
    !found.includes(item),
    'default (unpadded) insert keeps the item confined to its own cell range',
  );
});
...
test('queryBBoxRaw: an item spanning multiple cells inside the query range appears once per matching cell', () => {
  ...
  assert.ok(
    count > 1,
    `item spanning multiple cells is returned once per matching cell (got ${count}) — queryBBoxRaw is documented as undeduped, callers dedupe themselves`,
  );
});
...
test('getNearby: finds an item co-located with the query point', () => {
  ...
  const nearby = grid.getNearby(51.5, -0.1, (x) => `${x.type}_${x.id}`);
  assert.ok(
    nearby.includes(item),
    'getNearby finds an item in the same cell as the query point',
  );
});
...
test('getNearby: finds an item in a neighboring cell (3x3 window)', () => {
  ...
  grid.insert(bboxAround(51.5 + cellSize, -0.1 + cellSize), item);

  const nearby = grid.getNearby(51.5, -0.1, (x) => `${x.type}_${x.id}`);
  assert.ok(
    nearby.includes(item),
    "getNearby's 3x3 neighborhood finds an item one cell away in either axis",
  );
});
...
test('getNearby: excludes an item well outside the 3x3 neighborhood', () => {
  ...
  grid.insert(bboxAround(52.0, 1.0), item); // ~150km away

  const nearby = grid.getNearby(51.5, -0.1, (x) => `${x.type}_${x.id}`);
  assert.ok(
    !nearby.includes(item),
    "getNearby excludes an item far outside the query point's neighborhood",
  );
});
...
test('getNearby: dedupes an item found via multiple overlapping cells to exactly one result', () => {
  ...
  grid.insert(bboxAround(51.5, -0.1), item, 1);
  ...
  assert.strictEqual(
    count,
    1,
    'getNearby returns each item at most once even when it occupies several scanned cells',
  );
});
...
test('getNearby: defaults to identity-based dedup when no idFn is given', () => {
  ...
  const nearby = grid.getNearby(51.5, -0.1);
  const count = nearby.filter((x) => x === item).length;
  assert.strictEqual(
    count,
    1,
    'getNearby with no idFn still dedupes by object identity, not just by omitting dedup entirely',
  );
});
...
test('non-square cell sizes: insert/query correctly use separate lat and lon cell sizes', () => {
  ...
  const item = { id: 'a' };
  grid.insert(bboxAround(51.5, -0.1, 0.001), item);
  ...
  const found = grid.queryBBoxRaw(bboxAround(51.5, -0.1 + 0.015, 0.001));
  assert.ok(
    found.length === 0 || found.includes(item),
    'sanity: query does not throw or misbehave with asymmetric cell sizes',
  );

  // Constructor with a single arg mirrors cellSizeLon = cellSizeLat.
  const squareGrid = new SpatialGrid(0.01);
  assert.strictEqual(
    squareGrid.cellSizeLon,
    squareGrid.cellSizeLat,
    'single-argument constructor defaults cellSizeLon to cellSizeLat',
  );
});
...
test('cell boundary: a point exactly on a cell edge is consistently found by an item on either side', () => {
  ...
  const boundaryLat = 51.5; // 5150 * 0.01, an exact cell edge
  const item = { type: 'node', id: 9 };
  grid.insert(bboxAround(boundaryLat, -0.1), item);
  ...
  const fromBelow = grid.getNearby(
    boundaryLat - 0.001,
    -0.1,
    (x) => `${x.type}_${x.id}`,
  );
  const fromAbove = grid.getNearby(
    boundaryLat + 0.001,
    -0.1,
    (x) => `${x.type}_${x.id}`,
  );
  assert.ok(
    fromBelow.includes(item),
    'getNearby finds a boundary-sitting item when queried from just below the boundary',
  );
  assert.ok(
    fromAbove.includes(item),
    'getNearby finds a boundary-sitting item when queried from just above the boundary',
  );
});
...
test('computeCellWindow: correctly bounds row and col window for a point', () => {
  ...
  const rows = 101;
  const cols = 101;
  const win = SpatialGrid.computeCellWindow(
    51.51,
    -0.09,
    50,
    bounds,
...
test('computeCellWindow: clamps at grid boundaries for corner points', () => {
  ...
  const rows = 50;
  const cols = 50;
  const winMin = SpatialGrid.computeCellWindow(
    51.5,
    -0.1,
    100,
    bounds,
  ...
  const winMax = SpatialGrid.computeCellWindow(
    51.52,
    -0.08,
    100,
    bounds,
