...
test('calcEmFog: single band at ceiling → 100', () => {
  closeTo(
    GSRAnalyzer.calcEmFog({ rssi_868: -30 }),
    100,
    0.01,
    'single ceiling → 100',
  );
...
test('calcEmFog: single band at floor → 0', () => {
  closeTo(
    GSRAnalyzer.calcEmFog({ rssi_868: -100 }),
    0,
    0.01,
    'single floor → 0',
  );
...
test('calcEmFog: value above ceiling is clamped to 100', () => {
  closeTo(
    GSRAnalyzer.calcEmFog({ rssi_868: 0 }),
    100,
    0.01,
...
test('calcEmFog: custom bandFloors shift the scale', () => {
  ...
  closeTo(
    GSRAnalyzer.calcEmFog({ rssi_868: -55 }, { 868: -80 }),
    50,
    0.5,
    'custom floor',
  );
...
test('_interpolateGPS: real fix rows get hasGps = true', () => {
  const rows = makeRows([
    { lat: 51.0, lon: 0, time: 0 },
    { lat: 52.0, lon: 0, time: 1 },
  ]);
  GSRCSVParser._interpolateGPS(rows);
  assert.ok(rows[0].hasGps === true);
  assert.ok(rows[1].hasGps === true);
...
test('GSRUI resolution and marking helpers function correctly', () => {
  const mockAnalyzer = {
    peaks: [{ time: 10, label: 'test' }],
    setPeakLabel() {},
  };
  const mockTrack = {
    id: 'track_123',
    analyzer: mockAnalyzer,
    hasUnsavedLabels: false,
  };
...
test('MapColors.ROAD_COLORS is populated and matches getColorForMetric', () => {
  ...
  assert.strictEqual(
    MapColors.getColorForMetric('roadClass', 'motorway'),
    '#ff0055',
  );
  assert.strictEqual(
    MapColors.getColorForMetric('roadClass', 'primary'),
    '#ff6600',
  );
  assert.strictEqual(
    MapColors.getColorForMetric('roadClass', 'unknown_class'),
    '#666666',
  ); // fallback
});
...
test('GSRAnalyzer peak detection helpers work correctly', () => {
  ...
  assert.strictEqual(
    recoveryIdx,
    5,
    'recovery index is at index 5 (value 0.4)',
  );

  const noiseHalfWin = 10;
  const metrics = analyzer._calculateShapeMetrics(
    vals,
    times,
    4,
    onsetIdx,
...
test('GSRAnalyzer.hasSpatialData correctly identifies tracks with and without GPS', () => {
  ...
  if (fs.existsSync(track27Path)) {
    ...
    assert.strictEqual(
      a27.hasSpatialData,
      false,
      'track 27 has no spatial data',
    );
    assert.strictEqual(
      a27.hasGps,
      false,
      'hasGps alias matches hasSpatialData',
    );
  }
