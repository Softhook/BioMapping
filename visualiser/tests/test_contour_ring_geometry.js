...
const {
  ContourRingGeometry,
} = require('../src/render/contour_ring_geometry.mjs');

// ── toLoop ───────────────────────────────────────────────────────────────
test('toLoop: empty input returns a zeroed loop shape without throwing', () => {
  assert.deepStrictEqual(ContourRingGeometry.toLoop([]), {
    points: [],
    length: 0,
    diag: 0,
  });
  assert.deepStrictEqual(ContourRingGeometry.toLoop(null), {
    points: [],
    length: 0,
    diag: 0,
  });
...
test('toLoop: assigns monotonically increasing arc-length `t` starting at 0', () => {
  const loop = ContourRingGeometry.toLoop([
    { lat: 0, lon: 0 },
    { lat: 0, lon: 1 },
    { lat: 1, lon: 1 },
  ]);
...
test('buildRectangleLoop: every point carries the grid value from its perimeter cell and an outward-facing normal', () => {
  ...
  for (const p of loop.points) {
    assert.ok(
      [1, 2, 3, 4, 6, 7, 8, 9].includes(p.val),
      `perimeter point should carry a perimeter grid value, got ${p.val}`,
    );
...
test('tangentExtrapolate: extrapStart=false / extrapEnd=false independently skip that end', () => {
  ...
  assert.strictEqual(
    ContourRingGeometry.tangentExtrapolate(pts, 1.0, false, true).length,
    pts.length + 2,
  );
  assert.strictEqual(
    ContourRingGeometry.tangentExtrapolate(pts, 1.0, true, false).length,
    pts.length + 2,
  );
  assert.strictEqual(
    ContourRingGeometry.tangentExtrapolate(pts, 1.0, false, false).length,
    pts.length,
  );
});
...
test('tangentExtrapolate: fewer than 2 points returns the input unchanged (or [] for falsy input)', () => {
  assert.deepStrictEqual(
    ContourRingGeometry.tangentExtrapolate([{ lat: 0, lon: 0 }], 1.0),
    [{ lat: 0, lon: 0 }],
  );
...
test('tangentExtrapolate: extrapolated tips extend further for a larger `diag` scale factor', () => {
  ...
  const distFromStart = (p) =>
    Math.hypot(p.lat - pts[0].lat, p.lon - pts[0].lon);
...
test('traceMaskBoundary: an all-valid grid (no null cells) produces no boundary segments', () => {
  ...
  const bounds = { minLat: 0, maxLat: 1, minLon: 0, maxLon: 1 };
  assert.strictEqual(
    ContourRingGeometry.traceMaskBoundary(grid, rows, cols, bounds).length,
    0,
  );
});
...
test('smoothLoopPoints: fewer than 3 points is returned unchanged', () => {
  ...
  assert.deepStrictEqual(
    ContourRingGeometry.smoothLoopPoints(null),
    [],
    'null input falls back to [] (points || []), not null',
  );
});
...
test('recomputeSmoothNormals: fewer than 3 points is returned unchanged', () => {
  ...
  assert.deepStrictEqual(
    ContourRingGeometry.recomputeSmoothNormals(points),
    points,
  );
});
...
test('recomputeSmoothNormals: every point gets a unit-length normal perpendicular to its local tangent', () => {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const dot =
      recomputed[i].normal.lat * tangent.lat +
      recomputed[i].normal.lon * tangent.lon;
    assert.ok(
      Math.abs(dot) < 1e-9,
      `normal at index ${i} should be perpendicular to the local tangent`,
    );
...
test('recomputeSmoothNormals: picks the perpendicular direction that best agrees with the original outward normal, not its opposite', () => {
  ...
  for (let i = 0; i < points.length; i++) {
    const dot =
      recomputed[i].normal.lat * points[i].normal.lat +
      recomputed[i].normal.lon * points[i].normal.lon;
    assert.ok(
      dot >= 0,
      `recomputed normal at index ${i} should point the same general way as the original`,
    );
  }
});
...
test("findInteriorHoles: a mask island fully inside a ring is detected as that ring's hole", () => {
  ...
  const islandLoop = ContourRingGeometry.toLoop([
    { lat: 4, lon: 4 },
    { lat: 4, lon: 6 },
    { lat: 6, lon: 6 },
    { lat: 6, lon: 4 },
  ]);
  const rectangleLoop = ContourRingGeometry.toLoop([
    { lat: -100, lon: -100 },
    { lat: -100, lon: 100 },
    { lat: 100, lon: 100 },
    { lat: 100, lon: -100 },
  ]);
  const holes = ContourRingGeometry.findInteriorHoles(
    [ring],
    [rectangleLoop, islandLoop],
  );
  assert.strictEqual(holes.length, 1);
  assert.strictEqual(
    holes[0].length,
    1,
    'should find exactly one hole for the one ring',
  );
});
...
test('findInteriorHoles: a mask loop comparable in size to the ring itself (its own boundary) is not misidentified as a hole', () => {
  ...
  const holes = ContourRingGeometry.findInteriorHoles(
    [ring],
    [rectangleLoop, sameLoop],
  );
  assert.strictEqual(
    holes[0].length,
    0,
    'a loop comparable in size to the ring should not be treated as its own hole',
  );
});
...
test('findInteriorHoles: a mask island outside the ring is not attributed to it', () => {
  ...
  const farAwayLoop = ContourRingGeometry.toLoop([
    { lat: 50, lon: 50 },
    { lat: 50, lon: 52 },
    { lat: 52, lon: 52 },
    { lat: 52, lon: 50 },
