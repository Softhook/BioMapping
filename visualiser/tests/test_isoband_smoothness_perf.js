...
const grid = Array.from({ length: rows }, (_, r) =>
  Array.from({ length: cols }, (_, c) => {
    const dr = r - 78,
      dc = c - 78; // peak centered essentially in the SE corner
...
assert(
  contours.length > 0,
  'Sanity: at least one contour level produced segments',
);
...
assert(
  fillPaths.length > 0,
  'At least one filled isoband polygon is produced for the edge-pinned peak',
);
...
console.log(
  `✓ ${fillPaths.length} filled isoband polygons at max grid/contour settings, all closed`,
);
...
assert(
  smoothFillCount > 0,
  'Filled isobands use smooth Bézier curves rather than straight-edged grid-cell tiles',
);
console.log(
  `✓ ${smoothFillCount}/${fillPaths.length} filled isoband polygons use smooth curves (no blocky tiling)`,
);
...
assert(
  fillPaths.length < 500,
  `Filled isoband polygon count stays small — proportional to contour rings, not grid cells (got ${fillPaths.length})`,
);

assert(
  elapsedMs < 2000,
  `Export completes in reasonable time at worst-case settings (${elapsedMs}ms)`,
);
console.log(
  `✓ Worst-case export (80x80 grid, 25 contour levels) completed in ${elapsedMs}ms`,
);
