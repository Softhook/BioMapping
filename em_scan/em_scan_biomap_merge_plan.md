...
## Open design calls to confirm before/while implementing

These aren't fully settled — flagging explicitly rather than deciding unilaterally:

1. **RF snapshot storage**: fetch `em_scan_rf_worker_get_snapshot()` fresh wherever needed (CSV row builder, render callback) rather than caching arrays on `Session`. This matches how GSR values are already fetched fresh (`gsr_sensor_get_raw()` etc.) rather than cached per-tick — avoids a second copy of the same data going stale in two places. **Recommended.**
...
3. **On-screen RF readout**: both existing recording layouts are already tight (GpsGsr's graph fills y=16-64 with a zoom label overlaid at y=62; GpsOnly's `render_gps_detail` stacks ~5 lines already reaching y≈60-70). There's no obviously free space for a 3rd data category. Options:
   - Add one compact numeric line (`"-72 -65 -80"`, fixed 815/868/915 order) at bottom-right of the GpsGsr graph, mirroring how the zoom label already overlays bottom-left — no new screen space needed, just a symmetric addition.
   - Omit any on-screen RF readout in Phase 1 entirely — the CSV has the data regardless of what's on screen, and this reduces Phase 1's risk/scope further.
   - **Needs a decision before Phase 3** — not required to unblock Phase 1 (CSV-only) or Phase 2 (calibration).
4. **Calibration wizard control-flow style**: em_scan.c's wizard is tick/timer-driven (`EmScanModeCalPrep`/`CalSampling` advance once per `EventTypeTick` over a 30s+20s window). BioMapping's existing GSR wizard (`run_calibration_wizard`) is a **blocking loop** (`furi_delay_ms()` calls directly in `calibration_wizard_measure()`). Recommend rewriting the RF wizard to match the GSR wizard's blocking-loop style rather than porting em_scan.c's state machine verbatim — it's the closer precedent and avoids standing up a second independent timer/tick loop for something that only runs for ~50 seconds. **This is a rewrite of the control flow, not a mechanical port — confirm before implementing.**

## Phasing

Four independently buildable/testable phases. Each ends with a `python3 -m ufbt` build from repo root before moving to the next — the only mechanical verification available here (no GUI test shims, no hardware-in-the-loop).
...
No calibration wizard in this phase — `em_scan_cal_load()` returns "not calibrated," RF logs raw RSSI against nothing else (no floor comparison needed for logging, only for the on-screen fog reference / header's "Band Floors" line, neither needed yet).
...
- Control flow: rewritten to the GSR wizard's blocking-loop style (open call #4), not em_scan.c's tick-driven state machine — reuses `em_scan_rf_dwell_band()` + `em_scan_cal_compute_stats()` + `em_scan_cal_max_floor_dbm[]` + `em_scan_cal_save()`, all unchanged pure logic.
- Options menu: append (don't renumber existing 0-6) — index 7 = "RF Scan" enabled/disabled toggle, index 8 = "RF Calibration" → `run_rf_calibration_menu(app)`, then re-push `options_render` afterward exactly like case 3 (GSR Calibration) already does. `OPTIONS_COUNT` 7 → 9.
...
- Resolve open call #3 first.
- If proceeding: extend `biomap_render_callback()` with the single additive numeric line described above, shown only when `s->rf_worker != NULL`. Keep additive, not a redesign.

**Verification**: build-only; actual layout can only be judged on hardware or a flashed screenshot — out of reach in this environment.
...
- Manual review: the 3-way header-selection branch, the 3 snprintf branches, and the Options index renumbering (7→9) for off-by-ones.
...
- `biomap_session.c`, `biomap.h`, `biomap_config.h`, `biomap_types.h`, `application.fam` (Phase 1)
- `biomap_gui.c`, `biomap_render.c`, new `biomap_rf_cal.c` (Phase 2)
- `em_scan/em_scan_rf.c/h`, `em_scan/em_scan_rf_worker.c/h`, `em_scan/em_scan_cal.c/h` (reused as-is, Phases 1-2)
- `em_scan/em_scan.c`, `em_scan/modules/`, `em_scan/application.fam` (deleted, Phase 4)
